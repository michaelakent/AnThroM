...
```{r}
events <- read_csv("/Users/michaelakent/OwnCloud/Technician/1_AnThrom/Experiment_2/task/pilot/events_pilot.csv")
events_cloud = c(4,5,14)
```
## 2.1 Load the prolific export file
```{r, echo = TRUE, message = FALSE}
DF.demo <- list.files("pilot",pattern="\\prolific*", full.names=TRUE) %>% 
  map_dfr(read_csv) %>%
  filter(status == "APPROVED")
```

```{r}
DF.exp2 <- list.files("pilot",pattern="\\PARTICIPANT*", full.names=TRUE) %>%
...
DF.rating <- DF.exp2 %>%
  ...
  mutate_at(vars(-response, -prolific_id),as.factor)
```
...
DF.rating %>%
  ...
  annotate("text", x = 0.75, y = 0.6, size = 3.5, label = "Preregistered exclusion criteria") +
...
DF.fa %>%
  ...
  summarise(acc = mean(acc),
            rt = mean(rt)) %>%
  mutate_at(vars(-acc, -rt, -prolific_id), as.factor) %>%
  ...
  geom_hline(yintercept = 0.5, color = "grey", linetype = 2) +
  annotate("text", x = 0.75, y = 0.53, size = 3.5, label = "Preregistered exclusion criteria") +
  scale_fill_manual(values=c("#0072B2", "#D55E00")) +
  theme(legend.position="none") +
  ylim(0, 1.3)
```
## 3.5 Missing data: 
Participants who do not finish the task or who have 5 or more missing responses for the False Belief task will be excluded.
...
```{r, echo = TRUE, message=FALSE}
DF.fa <- dir_ls(paste("/Users/michaelakent/OwnCloud/Technician/1_AnThrom/Experiment_2/data/pilot"), regexp = "\\.csv$") %>% 
...
```{r, echo = TRUE, message=FALSE}
idaq <- read_csv("/Users/michaelakent/OwnCloud/Technician/1_AnThrom/Experiment_2/task/idaq_scale.csv") 

#load data (/Volumes/Project0255/dataset_1/sourcedata/)
DF.idaq  <- read_csv(file = "/Users//michaelakent/OwnCloud/Technician/1_AnThrom/Experiment_2/data/IDAQ_test.csv") %>%
