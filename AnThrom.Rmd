...
DF.idaq <- bind_rows(DF.d1, DF.d2, DF.d3, DF.d4, .id = "dataset") %>%
  ...
  transform(sub=as.integer(sub), dataset=as.integer(dataset))  
...
```{r}
DF.roi <- left_join(DF.roi, DF.idaq, by = c("sub","dataset"), keep = FALSE) %>%
...
cormat <- round(cor(as.matrix(cormat[3:17]), use = "complete.obs"), digits=3)
