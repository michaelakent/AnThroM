...
```
# 7. IDAQ and ROI Analysis - Overall
...
```{r}
DF.test <- DF.roi  %>% filter(roi == "mmpfc")

x <- DF.test$cent_IDAQ
y <- DF.test$cent_contrast
...
plot (x, y,
abline(LF1, col="red", lwd=2))
```

# 8. IDAQ and ROI Analysis - Overall

## 8.1 Combine IDAQ scores and ROI data
```{r}
DF.roi2 <- DF.idaq %>% 
  ungroup() %>% 
  mutate(dataset = as.numeric(dataset)) %>% 
...
```{r}
DF.roi2$cent_animal <- scale(DF.roi2$animal, scale = FALSE)
DF.roi2$cent_nature<- scale(DF.roi2$nature, scale = FALSE)
DF.roi2$cent_technology<- scale(DF.roi2$technology, scale = FALSE)

DF.roi2 <- DF.roi2 %>% group_by(roi) %>% mutate(cent_contrast = scale(contrast, scale =FALSE))
...
plot1 <- DF.roi2 %>%
  filter(network == "tom") %>%
  ggplot(aes(x=cent_nature, y=cent_contrast, colour=roi)) +
  geom_point(alpha=0.5,show.legend = FALSE) +
  geom_smooth(method="lm", se=FALSE, show.legend = FALSE) +
  labs(
    x="IDAQ score",
    ...
    facet_grid(dataset~roi)

plot1
```

## 8.4 Non-linear relationship
