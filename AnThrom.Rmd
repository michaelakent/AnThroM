...
```{bash}
docker run --rm -it -v /Volumes/Project0255/:/base nipy/heudiconv:latest -d /base/dataset_4/sourcedata/sub-{subject}/*.IMA -o /base/dataset_4 -f /base/code/heuristic_anthrom2.py -s 401 -c dcm2niix -b --overwrite
```
...
for subj in 315 316 317 318 319 320 321 322; do
...
```{bash}
singularity run --cleanenv /analyse/Project0255/my_images/fmriprep-1.5.2.simg /analyse/Project0255/dataset_4/ /analyse/Project0255/dataset_4/derivatives participant --participant-label sub-401 --fs-license-file /analyse/Project0255/my_images/license.txt --skip_bids_validation --fs-no-reconall -w /analyse/Project0255/work/dataset_4
```
...
DF.full <- bind_rows(DF.dataset1, DF.dataset4, .id = "dataset") %>%
...
library("psych")
bind_rows(DF.dataset1, DF.dataset4, .id = "dataset") %>% 
