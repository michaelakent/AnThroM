...
Before fitting any model we establish which link function we need to use. This code is taken from [Kevin Stadler's github]( https://kevinstadler.github.io/blog/bayesian-ordinal-regression-with-random-effects-using-brms/). In short, it uses the ordinal packadge to (quickly) find the link function that fits the data:
```{r}
library(ordinal)
...
6. Model comparison: 
```{r}
modelC1 <- LOO(ord.1, ord.2, ord.3, ord.4, ord.5, reloo = TRUE)
