...
rtpj %>% 
  ...
  ppc_stat_grouped(y = b_cent_IDAQ2,
                   stat = "median")
...
rope_range(rtpj) # determine the ROPE range

test <- bayestestR::equivalence_test(rtpj, range = c(-0.04232769, 0.04232769), ci = 0.95) # assert the null
test <- test[-c(3),]

plot(test) + theme_abyss() + scale_fill_flat() # plot the decision on H0
