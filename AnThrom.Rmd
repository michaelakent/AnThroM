...
#create variables
#not including subject 201 and 202 for now 
...
all_tom_roi <- bind_rows(DF.d1, DF.d2, DF.d3, DF.d4, .id = "dataset") %>%
  select(-bind) %>%
  select(8,1,2:7) 
```
...
p <- all_tom_roi %>%
  ...
  scale_colour_brewer(palette = "Blues") + ggtitle(paste("Comparison of contrasts values between datasets")) +
  facet_wrap(~roi)
p
```
...
all_pain_roi <- bind_rows(DF.d1, DF.d2, DF.d3, DF.d4, .id = "dataset") %>%
  select(-bind) %>%
  select(9,1,2:8) 
```
