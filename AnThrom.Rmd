...
```{bash}
docker run --rm -it -v /Volumes/Project0255/:/base nipy/heudiconv:latest -d /base/dataset_4/sourcedata/sub-{subject}/*.IMA -o /base/dataset_4 -f convertall -s 999 -c none --overwrite
```
...
```{bash}
docker run --rm -it -v /Volumes/Project0255/:/base nipy/heudiconv:latest -d /base/dataset_1/sourcedata/sub-{subject}/ses-{session}/*.dcm -o /base/dataset_1 -f convertall -s 101 -ss 01 -c none --overwrite
```
...
def infotodict(seqinfo):
    ...
    for idx, s in enumerate(seqinfo):
        ...
        if (s.dim4 == 175) and ('ep2d_ToM_Loc_boldTR2' in s.protocol_name):
            info[func_movie].append(s.series_id)
        if ('BP_ep2d_multiecho_32ch_p3_TOM' in s.protocol_name):
            info[func_movie].append(s.series_id)
...
```{bash}
docker run --rm -it -v /Volumes/Project0255/:/base nipy/heudiconv:latest -d /base/dataset_4/sourcedata/sub-{subject}/*.IMA -o /base/dataset_4 -f /base/code/heuristic_anthrom.py -s 999 -ss 01 -c dcm2niix -b --overwrite
```

For dataset_1 (for dataset_1 add ses-{session}/ and --ss 01 and .dcm). Movie for sub-101 and 102 is in ses-02. 
```{bash}
docker run --rm -it -v /Volumes/Project0255/:/base nipy/heudiconv:latest -d /base/dataset_1/sourcedata/sub-{subject}/ses-{session}/*.dcm -o /base/dataset_1 -f /base/code/heuristic_anthrom.py -s 116 127 -ss 01 -c dcm2niix -b --overwrite
```
...
```{bash}
singularity run -B /analyse/Project0255/:/base /analyse/Project0255/my_images/heudiconv_latest.sif -d /base/dataset_3/sourcedata/sub-{subject}*.dcm -o /base/dataset_3/ -f /base/code/heuristic_anthrom.py -s 313 -c dcm2niix -b --overwrite
```
=======
singularity run -B /autofs/space/piper_003/users/path/to/my/files/:/base /usr/pubsw/packages/heudiconv/heudiconv.simg -d /base/dicom/{subject}/* -o /base/nifti/ -f convertall -s mysubject080 -c none

>>>>>>> 87cec24130480075aaa66c0b0c6909abd5fcd5db
...
rootfolder=/Volumes/Project0255/dataset_2

for subj in 201 202; do
...
for subj in 104 105 106 108 109 110 111 115 116 120 121 125 126 127; do
	echo "Defacing participant $subj"
...
## 2.5 Update the .json file for the fmaps for dataset_3 and dataset_4

```{r}
library("rjson")

for (sub in 301:322){
json_data <- fromJSON(file=paste("/Volumes/Project0255/dataset_3/sub-", sub, "/fmap/sub-", sub, "_phasediff.json", sep = ""))
json_data$IntendedFor <- paste('func/sub-', sub, '_task-movie_bold.nii.gz',sep = "")
exportJson <- toJSON(json_data)
write(exportJson, file=paste("/Volumes/Project0255/dataset_3/sub-", sub, "/fmap/sub-", sub, "_phasediff.json", sep = ""))
}
```
...
```{bash}
docker run -ti --rm -v /Volumes/Project0255/dataset_1:/data:ro bids/validator /data
```
...
```{bash}
docker run -it --rm -v /Volumes/Project0255/dataset_1/:/data:ro -v /Volumes/Project0255/dataset_1/derivatives/mriqc:/out poldracklab/mriqc:0.14.2 /data /out participant --participant-label 201 -m T1w bold --ica --fft-spikes-detector 
```

On the grid do (cd in /analyse folder):

```{bash}
singularity run --cleanenv /analyse/Project0255/my_images/mriqc-0.14.2.simg /analyse/Project0255/dataset_2 /analyse/Project0255/dataset_2/derivatives/mriqc participant --participant-label 201 -m T1w bold --ica --fft-spikes-detector -w /analyse/Project0255/work
```
...
```{r}
#load own data
DF.dataset1  <- read_tsv(file = paste("/Volumes/Project0246/derivatives/mriqc/group_", modality, ".tsv", sep ="")) %>%
  separate(bids_name, c("sub", "session", "task", "run", "modality"), sep = "_", remove = FALSE) %>% #run and modality can be removed
  gather("measure", "value", 7:51) %>% 
  filter(measure == QCmeasure) 

#load api data
DF.dataset2 <- read_tsv(file = paste("/Volumes/Project0246/derivatives/mriqc/group_", modality, ".tsv", sep ="")) %>%
  separate(bids_name, c("sub", "session", "task", "run", "modality"), sep = "_", remove = FALSE) %>% #run and modality can be removed
  gather("measure", "value", 7:51) %>% 
  filter(measure == QCmeasure)

#combine data
DF.full <- bind_rows(DF.dataset1, DF.dataset2, .id = "data") %>%
  mutate(task = replace_na(task, "api"))
...
ggplot(DF.full,aes(x=task,y=value,fill=task))+
  geom_flat_violin(position=position_nudge(x = .2, y = 0),adjust =2, trim = FALSE, alpha = .5, colour = NA)+
  geom_point(aes(colour = task), position=position_jitter(width = .05), size = .5, shape = 20)+
  geom_boxplot(aes(x=task,y=value),position=position_nudge(x = .1, y = 0),outlier.shape = NA, alpha = .5, width = .1, colour = "black")+ 
  theme_classic() + ylab(QCmeasure) + scale_fill_brewer(palette = "Reds") +
  scale_colour_brewer(palette = "Reds") + ggtitle(paste("Comparison of", modality, "QC measure", QCmeasure, "with API dataset"))
```
...
fMRIprep version 1.5.1 was used on a local iMac.
...
```{bash}
/Users/soba/.local/bin/fmriprep-docker /Volumes/Project0255/dataset_1/ /Volumes/Project0255/dataset_1/derivatives participant --participant-label 114  --fs-license-file /Users/soba/license.txt --fs-no-reconall --use-syn-sdc --skip_bids_validation --func_only
```

singularity build /my_images/fmriprep-1.5.1.simg docker://poldracklab/fmriprep:1.5.1
...
#dataset_1
for (sub in 101:128){ #note: localisers for sub-101 and sub-102 are in ses-02
...
#dataset_3
for (sub in 301:321){ #see comment above for sub-322
...
## 4.1 First-level fROI analysis 
Extract data using marsbar. 
