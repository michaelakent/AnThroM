...
## 7.4 Create the quadratic predictor:
```{r}
...
```{r}
DF.test <- DF.roi %>% 
  select(sub, dataset, network, cent_contrast, cent_IDAQ, cent_IDAQ2) %>%
  filter(roi == "rtpj" | roi == "ltpj") %>%
  pivot_wider(names_from = roi, values_from = cent_contrast)

DF.test <- DF.roi %>% 
  filter(roi == "rtpj")

test <- brm(formula = ltpj ~ rtpj + cent_IDAQ2, 
             data = DF.test)


reg.1 <- brm(formula = cent_contrast ~ cent_IDAQ + cent_IDAQ2 + dataset, 
             data = DF.test, family = gaussian())
...
# linear model
    fit_lin <- run_model(
      brm(
        rating ~ time + Age + Geschlecht + Erstbehandlung + diag + 
          (time | PATNR) + (1 | item), 
        data = dat_tmp, family = fam, 
        cores = cores, chains = chains
      ),
      path = paste0("models/fit_hyp1_", meas, "_", fam, "_lin")
    )
