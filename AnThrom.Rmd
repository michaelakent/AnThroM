...
#load own data
DF.dataset1  <- read_csv(file = paste("/Volumes/Project0255/dataset_1/sourcedata/IDAQ_dataset1.csv", sep ="")) %>%
  gather("sub", "value", 3:31) 
DF.dataset2  <- read_csv(file = paste("/Volumes/Project0255/dataset_2/sourcedata/IDAQ_dataset2.csv", sep ="")) %>%
  gather("sub", "value", 3:37) %>%group_by( sub, Subscale) %>%
  filter(!is.na(value))%>%
  summarise(score = sum(value, na.rm = TRUE))%>%
  ungroup() %>%
  filter(Subscale == "IDAQ")
DF.dataset3  <- read_csv(file = paste("/Volumes/Project0255/dataset_3/sourcedata/IDAQ_dataset3.csv", sep ="")) %>%
  gather("sub", "value", 3:24) 
DF.dataset4  <- read_csv(file = paste("/Volumes/Project0255/dataset_4/sourcedata/IDAQ_dataset4.csv", sep ="")) %>%
  gather("sub", "value", 3:24) 

DF.full <- bind_rows(DF.dataset1, DF.dataset2, DF.dataset3, DF.dataset4, .id = "dataset") %>%
  group_by(dataset, sub, Subscale) %>%
  filter(!is.na(value))%>%
  summarise(score = sum(value, na.rm = TRUE)) 

```

## 5.2 IDAQ per dataset

Visualise the scores
```{r}
#create raincloud plot (check out the [github](https://github.com/RainCloudPlots/) or [preprint](https://wellcomeopenresearch.org/articles/4-63/v1)
p <- ggplot(DF.full,aes(x=dataset,y=score,fill=dataset))+
  ...
  theme_classic() + ylab(paste("score (0-150)")) + scale_fill_brewer(palette = "Blues") +
  scale_colour_brewer(palette = "Blues") + ggtitle(paste("Comparison of IDAQ scores between datasets")) +
  facet_wrap(~Subscale)

p
```

Median and interquartile range per dataset
```{r}
DF.full %>%
...
## 5.3 Calculate reliability of IDAQ
...
## 5.4 Comparison across datasets
```{r}
library("ez")
DF.full %>% ezANOVA(dv = score, wid = sub, within = .(Subscale), between = .(dataset))
```


# 6. fROI results {.tabset}

## 6.1 Comparison of ToM network contrast estimates across studies
...
tom_rois <- c("DMPFC","LTPJ","MMPFC","PC","RTPJ","VMPFC")

#load the fROI data
roi_d1 <- list.files(path = "/Volumes/Project0255/dataset_1/derivatives/roi/tom", pattern = "*.tsv", full.names= TRUE)
DF.d1 <- sapply(roi_d1, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="bind") %>%
  rename_at(2:7, ~tom_rois) %>%
  mutate(sub = c(101:129)) %>%
  mutate(dataset = 1)

roi_d2 <- list.files(path = "/Volumes/Project0255/dataset_2/derivatives/roi/tom", pattern = "*.tsv", full.names= TRUE)
DF.d2 <- sapply(roi_d2, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="bind") %>%
  rename_at(2:7, ~tom_rois) %>%
  mutate(sub = c(203:235))%>%
  mutate(dataset = 2)

roi_d2.extra <- list.files(path = "/Volumes/Project0255/dataset_2/derivatives/roi/tom/201_202", pattern = "*.tsv", full.names= TRUE)
DF.d2.extra <- sapply(roi_d2.extra, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="bind") %>%
  rename_at(2:7, ~tom_rois) %>%
  mutate(sub = c(201:202)) %>%
  mutate(dataset = 2)

roi_d3 <- list.files(path = "/Volumes/Project0255/dataset_3/derivatives/roi/tom", pattern = "*.tsv", full.names= TRUE)
DF.d3 <- sapply(roi_d3, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="bind") %>%
  rename_at(2:7, ~tom_rois) %>%
  mutate(sub = c(301:322)) %>%
  mutate(dataset = 3)

roi_d4 <- list.files(path = "/Volumes/Project0255/dataset_4/derivatives/roi/tom", pattern = "*.tsv", full.names= TRUE)
DF.d4 <- sapply(roi_d4, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="bind") %>%
  rename_at(2:7, ~tom_rois) %>%
  mutate(sub = c(401:422)) %>%
  mutate(dataset = 4)

all_tom_roi <- bind_rows(DF.d1, DF.d2.extra, DF.d2, DF.d3, DF.d4) %>%
  select(-bind) %>%
  select(8,7,1:6) 
```
...
p <- all_tom_roi %>%
  gather(roi, contrast, 3:8) %>%
  ggplot(.,aes(x=dataset,y=contrast,fill=dataset))+
  geom_flat_violin(position=position_nudge(x = .2, y = 0),adjust =2, trim = FALSE, alpha = .75, colour = NA)+
  geom_point(aes(colour = dataset), position=position_jitter(width = .05), size = .5, shape = 20)+
  geom_boxplot(aes(x=dataset,y=contrast),position=position_nudge(x = .1, y = 0),outlier.shape = NA, alpha = .5, width = .1, colour = "black")+ 
  theme_classic() + ylab(paste("contrast (mental vs pain)")) + scale_fill_brewer(palette = "Blues") +
  scale_colour_brewer(palette = "Blues") + ggtitle(paste("Comparison of ToM network contrasts values between datasets")) +
  facet_wrap(~roi)
p
...
## 6.2 Comparison of Pain Matrix contrast estimates across studies

Data wrangling
```{r}
#create variables
pain_rois <- c("AMCC","LINSULA","LMFG","LS2","RINSULA","RMFG","RS2")

#load the fROI data
pain_roi_d1 <- list.files(path = "/Volumes/Project0255/dataset_1/derivatives/roi/pain", pattern = "*.tsv", full.names= TRUE)
DF.d1 <- sapply(pain_roi_d1, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="bind") %>%
  rename_at(2:8, ~pain_rois) %>%
  mutate(sub = c(101:129)) %>%
  mutate(dataset = 1)

roi_d2.extra <- list.files(path = "/Volumes/Project0255/dataset_2/derivatives/roi/pain/201_202", pattern = "*.tsv", full.names= TRUE)
DF.d2.extra <- sapply(roi_d2.extra, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="bind") %>%
  rename_at(2:8, ~pain_rois) %>%
  mutate(sub = c(201:202)) %>%
...
pain_roi_d2 <- list.files(path = "/Volumes/Project0255/dataset_2/derivatives/roi/pain", pattern = "*.tsv", full.names= TRUE)
DF.d2 <- sapply(pain_roi_d2, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="bind") %>%
  rename_at(2:8, ~pain_rois) %>%
  mutate(sub = c(203:235)) %>%
  mutate(dataset = 2)

pain_roi_d3 <- list.files(path = "/Volumes/Project0255/dataset_3/derivatives/roi/pain", pattern = "*.tsv", full.names= TRUE)
DF.d3 <- sapply(pain_roi_d3, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="bind") %>%
  rename_at(2:8, ~pain_rois) %>%
  mutate(sub = c(301:322)) %>%
  mutate(dataset = 3)

pain_roi_d4 <- list.files(path = "/Volumes/Project0255/dataset_4/derivatives/roi/pain", pattern = "*.tsv", full.names= TRUE)
DF.d4 <- sapply(pain_roi_d4, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="bind") %>%
  rename_at(2:8, ~pain_rois) %>%
  mutate(sub = c(401:422)) %>%
  mutate(dataset = 4)

all_pain_roi <- bind_rows(DF.d1, DF.d2.extra, DF.d2, DF.d3, DF.d4) %>%
  select(-bind) %>%
  select(9,8,1:7) 
```
...
```{r}
p <- all_pain_roi %>%
  gather(roi, contrast, 3:9) %>%
  ggplot(.,aes(x=dataset,y=contrast,fill=dataset))+
  geom_flat_violin(position=position_nudge(x = .2, y = 0),adjust =2, trim = FALSE, alpha = .75, colour = NA)+
  geom_point(aes(colour = dataset), position=position_jitter(width = .05), size = .5, shape = 20)+
  geom_boxplot(aes(x=dataset,y=contrast),position=position_nudge(x = .1, y = 0),outlier.shape = NA, alpha = .5, width = .1, colour = "black")+ 
  theme_classic() + ylab(paste("contrast (mental vs pain)")) + scale_fill_brewer(palette = "Blues") +
  scale_colour_brewer(palette = "Blues") + ggtitle(paste("Comparison of Pain Matrix contrasts estimates between datasets")) +
  facet_wrap(~roi)
p
...
idaq_full <- DF.full %>% 
  ...
  spread(key=Subscale, value=score) #to separate idaq and idaq-na, e.g. wide format


#tidy data into long format

idaq_roi <- left_join(all_tom_roi, idaq_full, by = "sub") %>%
  select(-dataset.y) %>%
  rename(dataset = dataset.x) %>%
  pivot_longer(c(DMPFC:VMPFC), names_to="roi",values_to="value")
...
idaq_roi_wide <- left_join(all_tom_roi, idaq_full, by = "sub") %>%
  select(-dataset.y) %>%
  rename(dataset = dataset.x) %>%
  transform(IDAQ=as.numeric(IDAQ)) 

cormat <- round(cor(as.matrix(idaq_roi_wide[3:9]), use = "complete.obs"), digits=3)
...
```{r}
plot1 <- idaq_roi %>%
  ggplot(aes(x=IDAQ, y=value, colour=roi)) +
...
plot3 <- idaq_roi %>%
  ggplot(aes(x=IDAQ, y=value, colour=roi)) +
  geom_point(alpha=0.5,show.legend = FALSE) +
  geom_smooth(method="lm", formula=y ~ x+ I(x^2), se=FALSE, show.legend=FALSE) +
  labs(
    x="IDAQ score",
    y="Contrast"
  ) +
