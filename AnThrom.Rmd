...
## 5.7 Differences between datasets
Test if there are differences in IDAQ and IDAQ-NA scores between datasets:
```{r}
library(lme4)
mod0 = lmer(value ~ scale + dataset + (1|itemnr) + (1|sub), DF.idaq, family = 'gaussian')
summary(mod0)
drop1(mod0, test='Chisq')

library(brms)
fit1 <- brm(value ~ scale + dataset + (1|itemnr) + (1|sub), 
            data = DF.idaq, family = poisson())
summary(fit1) 
plot(fit1) 
pp = brms::pp_check(fit1)
pp + theme_bw()
```
...
DF.test <- DF.roi %>%
  filter(roi == "rtpj")
qplot(cent_IDAQ, cent_contrast, data=DF.test)
glm_post1 <- stan_glm(cent_IDAQ~cent_contrast, data=DF.roi, family=gaussian)
glm_post1 <- stan_glm(cent_IDAQ~cent_contrast + I(cent_IDAQ^2), data=DF.roi, family=gaussian)

stan_trace(glm_post1, pars=c("(Intercept)","cent_contrast","sigma"))
summary(glm_post1)
pp_check(glm_post1)

glm_fit <- glm(cent_IDAQ~cent_contrast +I(cent_IDAQ^2), data=DF.test, family=gaussian)
summary(glm_fit)
