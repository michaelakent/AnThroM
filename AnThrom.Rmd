...
Create the info file (for dataset_1 add ses-{session}/ and --ss 01 and .dcm)
```{bash}
...
docker run --rm -it -v /Volumes/Project0210/SHAREDBOTS/data:/base nipy/heudiconv:latest -d /base/sourcedata/sub-{subject}/ses-{session}/*.IMA -o /base/code -f convertall -s 01 -ss 01 -c none --overwrite
...
def infotodict(seqinfo):
    ...
    for idx, s in enumerate(seqinfo):
        ...
        if (s.dim4 == 175) and ('ToM_PartlyCloudy SENSE' in s.protocol_name):
...
Use the heuristic file to convert the Dicom files to .nii.gz (nifti) and create .json files  (for dataset_1 add ses-{session}/ and --ss 01 and .dcm).
