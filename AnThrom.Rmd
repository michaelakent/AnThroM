...
DF.test = DF.idaq %>% 
  #filter(!sub %in% sub_ex) %>% 
  filter(scale == "IDAQ") %>% 
  mutate(value = factor(value, levels=c(0:10), ordered=TRUE)) %>%
...
library(ordinal)
DF.test$value <- factor(DF.test$value)
...
ord.2 <- clm(value ~ dataset, nominal= ~dataset, data = DF.test)
summary(ord.2)

library(brms)
options(mc.cores = parallel::detectCores()) #run on multiple cores

ord.3 <- brm(formula  = value ~ dataset + (1|sub),  
             data  = DF.test,  
