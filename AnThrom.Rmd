...
ord.2 <- brm(
  value ~ 1 + dataset +
    (1|sub) + (1|itemnr),   
  data  = DF.test,  
  family  = cumulative("probit") # or "logit"
) 
...
Rerun winning model excluding the participants that completed the incorrect version: 
```{r}
DF.test = DF.test %>% 
  filter(!sub %in% sub_ex) 
