...
Before fitting any model we establish which link function we need to use? This code taken from [Kevin Stadler's github]( https://kevinstadler.github.io/blog/bayesian-ordinal-regression-with-random-effects-using-brms/). In short, it uses the ordinal packadge to find the link function that fits the data:
```{r}
library(ordinal)
...
saveRDS(ord.1, 'ord.1~simple.RDS') #save it to save time   
...
conditional_effects(ord.1t, "dataset", categorical = TRUE)
```
...
ord.2 <- brm(
  value ~ 1 + dataset +
    (1|sub) + (1|itemnr),   
  data  = DF.test,  
  family  = cumulative("logit") # or "logit"
) 
...
4. Category-specific model with unequal variances:
```{r}
ord.4 <- brm(
  value ~ 1 + dataset +
    (1|sub) + (1|itemnr),
...
saveRDS(ord.2E, 'ord.2E~random.RDS')  
ord.2E = readRDS('ord.2E~random.RDS')
```
...
test <- DF.temp %>% pivot_wider(names_from = c(network, roi), values_from = contrast) %>% write_csv("test.csv")
```
