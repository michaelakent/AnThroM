...
Dataset 2: 1-10 for 206-212, 219, 221-22, 224-25, 228, 231, 233-34
...
```{r}
#create raincloud plot (check out the [github](https://github.com/RainCloudPlots/) or [preprint](https://wellcomeopenresearch.org/articles/4-63/v1)
p <- ggplot(DF.idaq,aes(x=dataset,y=score,fill=dataset))+
...
#IDAQ
test <- bind_rows(DF.d1, DF.d2, DF.d3, DF.d4, .id = "dataset") %>% 
...
#IDAQ-NA
bind_rows(DF.d1, DF.d2, DF.d3, DF.d4, .id = "dataset") %>% 
...
## 6.1 Comparison of contrast estimates across studies and networks
...
#load the fROI data
temp <- list.files(path = "/Volumes/Project0255/dataset_1/derivatives/roi/pain", pattern = "*.tsv", full.names= TRUE)
DF.d1 <- sapply(temp, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="bind") %>%
  rename_at(2:8, ~pain_rois) %>%
  mutate(sub = c(101:129)) %>%
  mutate(dataset = 1)

temp <- list.files(path = "/Volumes/Project0255/dataset_2/derivatives/roi/pain/201_202", pattern = "*.tsv", full.names= TRUE)
DF.d2.extra <- sapply(temp, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="bind") %>%
  rename_at(2:8, ~pain_rois) %>%
  mutate(sub = c(201:202)) %>%
  mutate(dataset = 2)

temp <- list.files(path = "/Volumes/Project0255/dataset_2/derivatives/roi/pain", pattern = "*.tsv", full.names= TRUE)
DF.d2 <- sapply(temp, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="bind") %>%
  rename_at(2:8, ~pain_rois) %>%
  mutate(sub = c(203:235)) %>%
  mutate(dataset = 2)

temp <- list.files(path = "/Volumes/Project0255/dataset_3/derivatives/roi/pain", pattern = "*.tsv", full.names= TRUE)
DF.d3 <- sapply(temp, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="bind") %>%
  rename_at(2:8, ~pain_rois) %>%
  mutate(sub = c(301:322)) %>%
  mutate(dataset = 3)

temp <- list.files(path = "/Volumes/Project0255/dataset_4/derivatives/roi/pain", pattern = "*.tsv", full.names= TRUE)
DF.d4 <- sapply(temp, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="bind") %>%
  rename_at(2:8, ~pain_rois) %>%
...
idaq_tom <- left_join(all_tom_roi, idaq_full, by = "sub") %>%
  select(-dataset.y) %>%
  rename(dataset = dataset.x) %>%
  pivot_longer(c(DMPFC:VMPFC), names_to="roi",values_to="value")

idaq_pain <- left_join(all_pain_roi, idaq_full, by = "sub") %>%
  select(-dataset.y) %>%
  rename(dataset = dataset.x) %>%
  pivot_longer(c(AMCC:RS2), names_to="roi",values_to="value")
...
cormat <- round(cor(as.matrix(idaq_roi_wide[3:10]), use = "complete.obs"), digits=3)
