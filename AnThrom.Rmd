...
## 2.6 Combine the dual-echo runs for dataset_4
...
```{bash}
docker run -it --rm -v /Volumes/Project0255/dataset_4/:/data:ro -v /Volumes/Project0255/dataset_4/derivatives/mriqc:/out poldracklab/mriqc:0.14.2 /data /out participant --participant-label 403 -m T1w bold --ica --fft-spikes-detector 
```
...
ggplot(DF.full,aes(x=dataset,y=value,fill=dataset))+
...
```{bash}
singularity run --cleanenv /analyse/Project0255/my_images/fmriprep-1.5.2.simg /analyse/Project0255/dataset_4/ /analyse/Project0255/dataset_4/derivatives participant --participant-label sub-01 --fs-license-file /analyse/Project0255/my_images/license.txt --skip_bids_validation --fs-no-reconall -w /analyse/Project0255/work/dataset_4
```
...
dataset1 <- list.files(path = "/Volumes/Project0255/dataset_1/derivatives/roi/tom", pattern = "*.tsv", full.names= TRUE)
tbl1 <- sapply(dataset1, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="test")
#want to rename the columns with tom_rois variable?

dataset2 <- list.files(path = "/Volumes/Project0255/dataset_2/derivatives/roi/tom", pattern = "*.tsv", full.names= TRUE)
tbl2 <- sapply(dataset2, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="test")

dataset3 <- list.files(path = "/Volumes/Project0255/dataset_3/derivatives/roi/tom", pattern = "*.tsv", full.names= TRUE)
tbl3 <- sapply(dataset3, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="test")

dataset4 <- list.files(path = "/Volumes/Project0255/dataset_4/derivatives/roi/tom", pattern = "*.tsv", full.names= TRUE)
tbl4 <- sapply(dataset4, read_tsv, col_names=FALSE, simplify=TRUE) %>%
  bind_rows(.id ="test")
...
#create raincloud plot (check out the [github](https://github.com/RainCloudPlots/) or [preprint](https://wellcomeopenresearch.org/articles/4-63/v1)
ggplot(DF.full,aes(x=dataset,y=score,fill=dataset))+
  geom_flat_violin(position=position_nudge(x = .2, y = 0),adjust =2, trim = FALSE, alpha = .5, colour = NA)+
  geom_point(aes(colour = dataset), position=position_jitter(width = .05), size = .5, shape = 20)+
