...
```{r}
library("ez")
idaq_ez <- DF.idaq %>% ezANOVA(dv = score, wid = sub, within = .(scale), between = .(dataset))
print(idaq_ez)
```
...
roi_extract <- function(datasetno, substart, subend, network, nroi) {
  ...
  dir_ls(paste("/Volumes/Project0255/dataset_", datasetno, "/derivatives/roi/", network, sep = ""), regexp = "\\.tsv$") %>% 
    ...
    mutate(id = str_extract(id, "dmpfc|mmpfc|vmpfc|ltpj|rtpj|prec|amcc|lmfg|rmfg|ls2|rs2|linsula|rinsula|Cullen_ltpj|Waytz_mpfc")) %>%
...
Function loading data from local project folder
```{r}
#create function to load roi datasets for the different networks
roi_extract2 <- function(datasetno, substart, subend, network, nroi) {
...
```{r}
DF.d1 <- roi_extract2(1, 101, 129, "tom", 6)
DF.d2.a <- roi_extract2(2, 201, 202, "tom/201_202", 6)
DF.d2.b <- roi_extract2(2, 203, 235, "tom", 6)
DF.d3 <- roi_extract2(3, 301, 322, "tom", 6)
DF.d4 <- roi_extract2(4, 401, 422, "tom", 6)
...
```{r}
DF.d1 <- roi_extract2(1, 101, 129, "pain", 7)
DF.d2.a <- roi_extract2(2, 201, 202, "pain/201_202/", 7)
DF.d2.b <- roi_extract2(2, 203, 235, "pain", 7)
DF.d3 <- roi_extract2(3, 301, 322, "pain", 7)
DF.d4 <- roi_extract2(4, 401, 422, "pain", 7)
...
Load the data for the other peak voxels taken from Waytz et al. (mPFC) and Cullen et al. (lTPJ)
```{r}
DF.other_1 <- roi_extract(1, 101, 129, "other", 2)
DF.other_2a <- roi_extract(2, 201, 202, "other/201_202/", 2)
DF.other_2b <- roi_extract(2, 203, 235, "other", 2)
DF.other_3 <- roi_extract(3, 301, 322, "other", 2)
DF.other_4 <- roi_extract(4, 401, 422, "other", 2)

DF.other_roi <- bind_rows(DF.other_1,DF.other_2a,DF.other_2b,DF.other_3,DF.other_4)
head(DF.other_roi)
```
...
DF.roi <- DF.idaq %>% 
  ...
  spread(key=scale, value=score) #change this


# Try for other rois (Waytz_mpfc and Cullen_ltpj)
DF.roi2 <- DF.idaq %>% 
  group_by(sub,dataset, scale) %>%
  summarise(score = sum(score)) %>%
  ungroup() %>% 
  mutate(dataset = as.numeric(dataset)) %>% 
  left_join(DF.other_roi, DF.idaq, by = c("sub","dataset"), keep = FALSE) %>%
  spread(key=scale, value=score)
```
...
linearmod4 <- lm(cent_IDAQ ~ cent_contrast, data=DF.roi)
summary(linearmod4)
```
...
## 7.5 Relationships with other voxels (Cullen_ltpj and Waytz_mpfc)
```{r}

# Linear 

other_roi_plot1 <- DF.roi2 %>%
  filter(network == "other") %>%
  ggplot(aes(x=IDAQ, y=contrast, colour=roi)) +
  geom_point(alpha=0.5,show.legend = FALSE) +
...
# 8. Load data for Bayesian analysis:
### Loading packages
```{r}
library(brms)
library(rstan)
...
# idea of brms: fitting all kinds of regression models within one framework 
# can use brm in the same way as lme4
#fit <- brm(Reaction ~ 1 + Days + (1 + Days|Subject),
#           data = sleepstudy,
#           family = x())

# for complex non-linear relationships something like this:
#  fitgp <- brm(y ~ s(x), bdata)
# marginal_effects(fitgp, nsamples = 100, spaghetti = TRUE)

#at1 <- brm(contrast ~ s(IDAQ), 
#           data = )
```
...
# roi data loaded as above (section 6.1) using roi extract function 2
```
...
DF.roi <- bind_rows(DF.d1, DF.d2.a, DF.d2.b, DF.d3, DF.d4) 
```

## 8.2 Combine IDAQ and ROI then center:
```{r}
DF.roi <- DF.idaq %>% 
  group_by(sub,dataset, scale) %>%
  summarise(score = sum(score)) %>%
...
DF.roi %>%
  ...
  geom_point(position = "jitter",
             alpha    = 0.5)+ #to add some random noise for plotting purposes
  theme_minimal()+
...
```{r}
DF.roi <- DF.roi %>%
  mutate(cent_IDAQ2 = cent_IDAQ^2)

model_tom <- brm(formula = cent_contrast ~ cent_IDAQ + cent_IDAQ2, 
             data    = DF.roi,
             seed    = 123,
             file="model_tom.RDS")
```
...
summary(model_tom)
plot(model_tom)
posterior_summary(model_tom)
saveRDS(model_tom, file="model_tom")
```
...
mcmc_areas(posterior,
           pars = c("b_IDAQ", "b_Intercept"),
           prob = 0.8) + plot_title
```


## 8.3 Model for seperate ROIs:
```{r}
DF.roi.sep <- DF.idaq %>% 
  group_by(sub, dataset, scale) %>%
  summarise(score = sum(score)) %>%
  ...
  left_join(DF.roi, DF.idaq, by = c("sub","dataset"), keep = FALSE) %>%
  spread(key=scale, value=score) %>%
  spread(roi, cent_contrast) 

DF.roi.sep <- DF.roi.sep %>%
  mutate(cent_IDAQ2 = cent_IDAQ^2)
```
...
DF.roi %>%
  group_by(roi) %>%
  summarise(mcontrast = mean(cent_contrast),
            mIDAQ = mean(cent_IDAQ),
            sdcontrast = sd(cent_contrast),
            sdIDAQ = sd(cent_IDAQ))
```

LTPJ:
```{r}
ltpjmodel <- brm(formula = ltpj ~ cent_IDAQ + cent_IDAQ2, 
             data = DF.roi.sep,
             seed = 123)

summary(ltpjmodel)
plot(ltpjmodel)
posterior_summary(ltpjmodel)
saveRDS(ltpjmodel, file = "ltpjmodel")

posterior_ltpj <- as.matrix(ltpjmodel)

mcmc_areas(posterior_ltpj,
...
RTPJ:
```{r}
rtpjmodel <- brm(formula = rtpj ~ IDAQ + IDAQ2, 
             data = DF.roi.sep,
             seed = 123,
             file = "rtpjmodel")
...
summary(rtpjmodel)
plot(rtpjmodel)
posterior_summary(rtpjmodel)
saveRDS(rtpjmodel,file = "rtpjmodel")

posterior_rtpj <- as.matrix(rtpjmodel)

mcmc_areas(posterior_rtpj,
           pars = c("b_IDAQ", "b_IDAQ2"),
           prob = 0.8) + plot_title
```

Compare LTPJ and RTPJ:
```{r}
tpjmod <- brm(formula = rtpj ~ ltpj,
              data = DF.roi.sep,
              seed = 123,
              file = "tpjmod.RDS")
...
summary(tpjmod)
plot(tpjmod)
posterior_summary(tpjmod)
saveRDS(tpjmod,file="tpjmod")
launch_shinystan(tpjmod)

tpjmod2 <- brm(formula = rtpj ~ltpj + IDAQ,
               data = DF.roi.sep, 
               seed = 123,
               file = "tpjmod2.RDS")
...
summary(tpjmod2)
plot(tpjmod2)
posterior_summary(tpjmod2)
saveRDS(tpjmod2, file = "tpjmod2")

plot(conditional_effects(tpjmod2), points = TRUE)
```


DMPFC:
```{r}
dmpfcmodel <- brm(formula = dmpfc ~ IDAQ + IDAQ2, 
             data = DF.roi.sep,
             seed = 123,
             file = "dmpfcmodel.RDS")
...
summary(dmpfcmodel)
plot(dmpfcmodel)
posterior_summary(dmpfcmodel)
saveRDS(dmpfcmodel, file = "dmpfcmodel")

posterior_dmpfc <- as.matrix(dmpfcmodel)

mcmc_areas(posterior_dmpfc,
           pars = c("b_IDAQ", "b_IDAQ2"),
           prob = 0.8) + plot_title
```
...
MMPFC:
```{r}
mmpfcmodel <- brm(formula = mmpfc ~ IDAQ + IDAQ2, 
             data = DF.roi.sep,
             seed = 123,
             file = "mmpfcmodel.RDS")
...
summary(mmpfcmodel)
plot(mmpfcmodel)
posterior_summary(mmpfcmodel)
saveRDS(mmpfcmodel, file = "mmpfcmodel")

posterior_mmpfc <- as.matrix(mmpfcmodel)

mcmc_areas(posterior_mmpfc,
           pars = c("b_IDAQ", "b_IDAQ2"),
           prob = 0.8) + plot_title
```
...
VMPFC:
```{r}
vmpfcmodel <- brm(formula = vmpfc ~ IDAQ + IDAQ2, 
             data = DF.roi.sep,
             seed = 123,
             file = "vmpfcmodel.RDS")
...
summary(vmpfcmodel)
plot(vmpfcmodel)
posterior_summary(vmpfcmodel)
saveRDS(vmpfcmodel, file = "vmpfcmodel")

posterior_vmpfc <- as.matrix(vmpfcmodel)

mcmc_areas(posterior_vmpfc,
           pars = c("b_IDAQ", "b_IDAQ2"),
           prob = 0.8) + plot_title
```
...
PREC:
```{r}
precmodel <- brm(formula = prec ~ IDAQ + IDAQ2, 
             data = DF.roi.sep,
             seed = 123,
             file = "precmodel.RDS")
...
summary(precmodel)
plot(precmodel)
posterior_summary(precmodel)
saveRDS(precmodel, file = "precmodel")

posterior_prec <- as.matrix(precmodel)


mcmc_areas(posterior_prec,
           pars = c("b_IDAQ", "b_IDAQ2"),
           prob = 0.8) + plot_title
```

## 8.4 Models using centered data:

LTPJ:
```{r}
ltpjmodel_c <- brm(formula = ltpj ~ cent_IDAQ + cent_IDAQ2, 
             data = DF.roi.sep,
             seed = 123,
             file = "ltpjmodel_c.RDS")

summary(ltpjmodel_c)
plot(ltpjmodel_c)
posterior_summary(ltpjmodel_c)
saveRDS(ltpjmodel_c, file = "ltpjmodel_c")

posterior_ltpj_c <- as.matrix(ltpjmodel_c)

plot_ltpj <- mcmc_areas(posterior_ltpj_c,
           pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),
           prob = 0.8) + plot_title
plot_ltpj
```
...
```{r}
rtpjmodel_c <- brm(formula = rtpj ~ cent_IDAQ + cent_IDAQ2, 
             data = DF.roi.sep,
             seed = 123,
             file = "rtpjmodel_c.RDS")

summary(rtpjmodel_c)
plot(rtpjmodel_c)
posterior_summary(rtpjmodel_c)
saveRDS(rtpjmodel_c, file = "rtpjmodel_c")

posterior_rtpj_c <- as.matrix(rtpjmodel_c)

plot_rtpj <- mcmc_areas(posterior_rtpj_c,
           pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),
           prob = 0.8) + plot_title
```
...
DMPFC:
```{r}
dmpfcmodel_c <- brm(formula = dmpfc ~ cent_IDAQ + cent_IDAQ2, 
             data = DF.roi.sep,
             seed = 123,
             file = "dmpfcmodel_c.RDS")

summary(dmpfcmodel_c)
plot(dmpfcmodel_c)
posterior_summary(dmpfcmodel_c)
saveRDS(dmpfcmodel_c, file = "dmpfcmodel_c")

posterior_dmpfc_c <- as.matrix(dmpfcmodel_c)

plot_dmpfc <- mcmc_areas(posterior_dmpfc_c,
           pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),
           prob = 0.8) + plot_title
...
MMPFC:
```{r}
mmpfcmodel_c <- brm(formula = mmpfc ~ cent_IDAQ + cent_IDAQ2, 
             data = DF.roi.sep,
             seed = 123,
             file = "mmpfcmodel_c.RDS")

summary(mmpfcmodel_c)
plot(mmpfcmodel_c)
posterior_summary(mmpfcmodel_c)
saveRDS(mmpfcmodel_c, file = "mmpfcmodel_c")

posterior_mmpfc_c <- as.matrix(mmpfcmodel_c)

plot_mmpfc <- mcmc_areas(posterior_mmpfc_c,
           pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),
           prob = 0.8) + plot_title
...
VMPFC:
```{r}
vmpfcmodel_c <- brm(formula = vmpfc ~ cent_IDAQ + cent_IDAQ2, 
             data = DF.roi.sep,
             seed = 123,
             file = "vmpfcmodel_c.RDS")

summary(vmpfcmodel_c)
plot(vmpfcmodel_c)
posterior_summary(vmpfcmodel_c)
saveRDS(vmpfcmodel_c, file = "vmpfcmodel_c")

posterior_vmpfc_c <- as.matrix(vmpfcmodel_c)

plot_vmpfc <- mcmc_areas(posterior_vmpfc_c,
           pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),
           prob = 0.8) + plot_title
```
...
PREC:
```{r}
precmodel_c <- brm(formula = prec ~ cent_IDAQ + cent_IDAQ2, 
             data = DF.roi.sep,
             seed = 123,
             file = "precmodel_c.RDS")

summary(precmodel_c)
plot(precmodel_c)
posterior_summary(precmodel_c)
saveRDS(precmodel_c, file = "precmodel_c")

posterior_prec_c <- as.matrix(precmodel_c)

plot_prec <- mcmc_areas(posterior_prec_c,
           pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),
           prob = 0.8) + plot_title
```
...
```{r}
#library(patchwork)
plot_ltpj + plot_rtpj + plot_prec + 
  plot_vmpfc + plot_dmpfc + plot_mmpfc
```

Plot using code adapted from https://www.rensvandeschoot.com/tutorials/r-linear-regression-bayesian-using-brms/
First want to combine:
```{r}
post_tom <- bind_rows("ltpj" = as_tibble(as.mcmc(ltpjmodel_c, pars = c("b_cent_IDAQ","b_cent_IDAQ2"), combine_chains = TRUE)),
                      "rtpj" = as_tibble(as.mcmc(rtpjmodel_c, pars = c("b_cent_IDAQ","b_cent_IDAQ2"), combine_chains = TRUE)),
                      "prec" = as_tibble(as.mcmc(precmodel_c, pars = c("b_cent_IDAQ","b_cent_IDAQ2"), combine_chains = TRUE)),
                      "vmpfc" = as_tibble(as.mcmc(vmpfcmodel_c, pars = c("b_cent_IDAQ","b_cent_IDAQ2"), combine_chains = TRUE)),
                      "dmpfc" = as_tibble(as.mcmc(dmpfcmodel_c, pars = c("b_cent_IDAQ","b_cent_IDAQ2"), combine_chains = TRUE)),
                      "mmpfc" = as_tibble(as.mcmc(mmpfcmodel_c, pars = c("b_cent_IDAQ","b_cent_IDAQ2"), combine_chains = TRUE)),
                      .id = "roi")


post_tom <- post_tom %>%
  gather(key = "IDAQmodel", value = "value", 2:3)
```

Using ggplot:
```{r}
ggplot(data = post_tom,aes(value, fill = IDAQmodel)) +
  geom_density(size = 1, alpha = 0.5, color = NA) +
  scale_fill_brewer() +
  theme_linedraw(base_size=9) +
  theme(panel.grid = element_blank()) +
  theme(strip.background = element_blank()) +
  theme(axis.line.y=element_line(), axis.text.y = element_blank()) +
  geom_vline(xintercept = 0, colour = "black", linetype = "dashed", alpha = 0.5) +
  coord_fixed(ratio = 1/50) +
  facet_wrap(~roi, ncol = 1) +
  labs(title = expression("Posterior distributions"), 
       subtitle = "Theory-of-Mind Network")
```

## 8.4 Creating a table with posterior information
```{r}
...
table_ltpj <- tab_model(ltpjmodel_c)
table_rtpj <- tab_model(rtpjmodel_c)
table_prec <- tab_model(precmodel_c)
table_dmpfc <- tab_model(dmpfcmodel_c)
table_mmpfc <- tab_model(mmpfcmodel_c)
table_vmpfc <- tab_model(vmpfcmodel_c)

tab_model(ltpjmodel_c,rtpjmodel_c,precmodel_c,dmpfcmodel_c,mmpfcmodel_c,vmpfcmodel_c)
