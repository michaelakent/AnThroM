...
library(fs)
library(patchwork) 
library(easystats) #remotes::install_github("easystats/easystats")
library(sjPlot)
library(httr)
library(kableExtra)
library(magrittr)
...
roi_extract <- function(datasetno, substart, subend, network, nroi) {
  ...
  dir_ls(paste("experiment1/dataset_", datasetno, "/derivatives/roi/", network, sep = ""), regexp = "\\.tsv$") %>% 
    ...
    mutate(network = str_extract(network, "tom|pain|other")) %>%
    mutate(id = str_extract(id, "dmpfc|mmpfc|vmpfc|ltpj|rtpj|prec|amcc|lmfg|rmfg|ls2|rs2|linsula|rinsula|rmpfc-acc")) %>%
...
```{r}
roi_sum <- summarySEwithin(DF.roi, measurevar="contrast", withinvars= c("roi","network"), betweenvars = "dataset", idvar="sub")
...
## 9.6 HDI+ROPE using ggplot based on [http://mjskay.github.io/tidybayes/articles/tidy-brms.html#posterior-predictions-kruschke-style-1]

Pain and tom networks
```{r}
get_variables(tom)

tom_pain <-
  bind_rows("tom" = as_tibble(as.mcmc(tom,
  pars = c( "b_cent_IDAQ","b_cent_IDAQ2"),
  combine_chains = TRUE)),
  "pain" = as_tibble(as.mcmc(pain,
  pars = c( "b_cent_IDAQ","b_cent_IDAQ2"),
  combine_chains = TRUE)),
  .id = "network")

modelnames <- list('tom'="tom",'pain'="pain")
...
tom_pain_gg <- tom_pain %>%
  pivot_longer(c("b_cent_IDAQ","b_cent_IDAQ2"),names_to = "predictor") %>%
  mutate(Decision= ifelse(network=="tom","Undecided",
                          ifelse(network=="pain","Accept","NA"))) %>%
  group_by("Decision")%>%
  ggplot(aes(y = predictor, x = value,fill=Decision)) +
  stat_halfeye(alpha=0.5) +
  geom_vline(xintercept = c(-.1, .1), linetype = "dashed") +
  labs(y = "Predictor", x="") +
  scale_y_discrete(labels=c("b_cent_IDAQ"="linear","b_cent_IDAQ2"="quadratic")) +
  theme_classic() +
  scale_fill_manual(values=colours2)+
  theme_set(theme_classic(base_size = 15))+
  facet_wrap(~network,labeller = model_labeller)

tom_pain_gg

ggsave("experiment1/figures/HDIROPE_PainTom.TIFF", width = 24,height = 16, plot = tom_pain_gg, units = "cm", dpi = 300)
```

All regions
```{r}

order2 <- c("rtpj", "ltpj", "prec", "vmpfc","mmpfc","dmpfc")  
...
modelnames <- list('rtpj'="rtpj",'ltpj'="ltpj",'prec'="prec",'dmpfc'="dmpfc",'vmpfc'="vmpfc",'mmpfc'="mmpfc")
model_labeller <- function(variable,value){
  return(modelnames[value])
}

tom_all_regions <-
  bind_rows("rtpj" = as_tibble(as.mcmc(rtpj,
  pars = c( "b_cent_IDAQ","b_cent_IDAQ2"),
  combine_chains = TRUE)),
  "ltpj" = as_tibble(as.mcmc(ltpj,
  pars = c( "b_cent_IDAQ","b_cent_IDAQ2"),
  combine_chains = TRUE)),
  "prec" = as_tibble(as.mcmc(prec,
  pars = c( "b_cent_IDAQ","b_cent_IDAQ2"),
  combine_chains = TRUE)),
  "vmpfc" = as_tibble(as.mcmc(vmpfc,
  pars = c( "b_cent_IDAQ","b_cent_IDAQ2"),
  combine_chains = TRUE)),
  "mmpfc" = as_tibble(as.mcmc(mmpfc,
  pars = c( "b_cent_IDAQ","b_cent_IDAQ2"),
  combine_chains = TRUE)),
  "dmpfc" = as_tibble(as.mcmc(dmpfc,
  pars = c( "b_cent_IDAQ","b_cent_IDAQ2"),
  combine_chains = TRUE)),
  .id = "region")

tom_all_gg <- tom_all_regions %>%
  pivot_longer(c("b_cent_IDAQ","b_cent_IDAQ2"),names_to = "predictor") %>%
  mutate(Decision= ifelse(region=="rtpj"|region=="ltpj"|region=="prec"|region=="vmpfc"|region=="mmpfc"|region=="dmpfc","Undecided",
                          ifelse("NA"))) %>%
  group_by("Decision")%>%
  mutate(region = as.factor(region),
            region = reorder_within(region, order2)) %>%
  ggplot(aes(y = predictor, x = value,fill=Decision)) +
  stat_halfeye(alpha=0.5) +
  geom_vline(xintercept = c(-.1, .1), linetype = "dashed") +
  labs(y = "Predictor", x="") +
  ...
  scale_fill_manual(values=colours2)+
  theme_set(theme_classic(base_size = 15))+
  facet_wrap(~region,labeller = model_labeller)

tom_all_gg

ggsave("experiment1/figures/HDIROPE_TomRegions.TIFF", width = 24,height = 16, plot = tom_all_gg, units = "cm", dpi = 300)
```
...
ggsave("experiment1/figures/s6update.TIFF", plot = s6, width = 24.7, height = 15.24, units = "cm", dpi = 300)
```
...
# 11. Further exploratory/control analyses {.tabset}

## 11.1 Hand-coded events

Load the Theory-of-Mind ROI data for the hand coded events (Jacoby et al., 2016):
```{r}
#mental vs. control
DF.d1 <- roi_extract(1, 101, 129, "control/mental_control", 6)
DF.d2.a <- roi_extract(2, 201, 202, "control/201_202/mental_control", 6)
DF.d2.b <- roi_extract(2, 203, 235, "control/mental_control", 6)
DF.d3 <- roi_extract(3, 301, 322, "control/mental_control", 6)
DF.d4 <- roi_extract(4, 401, 422, "control/mental_control", 6)

DF.mental <- bind_rows(DF.d1, DF.d2.a, DF.d2.b, DF.d3, DF.d4) %>%
  mutate_at(vars(-contrast),as.factor) %>% 
  dplyr::rename(comparison = network) %>% 
  mutate(comparison = "mental_control")

#pain vs. control
DF.d1 <- roi_extract(1, 101, 129, "control/pain_control", 6)
DF.d2.a <- roi_extract(2, 201, 202, "control/201_202/pain_control", 6)
DF.d2.b <- roi_extract(2, 203, 235, "control/pain_control", 6)
DF.d3 <- roi_extract(3, 301, 322, "control/pain_control", 6)
DF.d4 <- roi_extract(4, 401, 422, "control/pain_control", 6)

DF.pain <- bind_rows(DF.d1, DF.d2.a, DF.d2.b, DF.d3, DF.d4) %>%
  mutate_at(vars(-contrast),as.factor) %>% 
  dplyr::rename(comparison = network) %>% 
  mutate(comparison = "pain_control")

#social vs. control
DF.d1 <- roi_extract(1, 101, 129, "control/social_control", 6)
DF.d2.a <- roi_extract(2, 201, 202, "control/201_202/social_control", 6)
DF.d2.b <- roi_extract(2, 203, 235, "control/social_control", 6)
DF.d3 <- roi_extract(3, 301, 322, "control/social_control", 6)
DF.d4 <- roi_extract(4, 401, 422, "control/social_control", 6)

DF.social <- bind_rows(DF.d1, DF.d2.a, DF.d2.b, DF.d3, DF.d4) %>%
  mutate_at(vars(-contrast),as.factor) %>% 
  dplyr::rename(comparison = network) %>% 
  mutate(comparison = "social_control")

#all vs. control
DF.d1 <- roi_extract(1, 101, 129, "control/all_control", 6)
DF.d2.a <- roi_extract(2, 201, 202, "control/201_202/all_control", 6)
DF.d2.b <- roi_extract(2, 203, 235, "control/all_control", 6)
DF.d3 <- roi_extract(3, 301, 322, "control/all_control", 6)
DF.d4 <- roi_extract(4, 401, 422, "control/all_control", 6)

DF.all<- bind_rows(DF.d1, DF.d2.a, DF.d2.b, DF.d3, DF.d4) %>%
  mutate_at(vars(-contrast),as.factor) %>% 
  dplyr::rename(comparison = network) %>% 
  mutate(comparison = "all_control") 

DF.hand<- bind_rows(DF.mental, DF.pain, DF.social, DF.all) %>% 
  group_by(sub, dataset) %>%
  mutate_at(vars(-contrast),as.factor) %>% 
  mutate(roi = fct_relevel(roi, order)) 
rm(DF.d1, DF.d2.a, DF.d2.b, DF.d3, DF.d4, DF.mental, DF.pain, DF.social, DF.all)
```


```{r}
roi_sum <- summarySEwithin(DF.hand, measurevar="contrast", withinvars= c("roi","comparison"), idvar="sub") 
...
FS2 <- DF.hand %>%
  ...
  geom_point(aes(colour = roi, fill = roi), position=position_jitter(width = .05), size = .5, shape = 21, colour = "Black") +
  geom_boxplot(aes(x=roi,y=contrast),position=position_nudge(x = .1, y = 0),outlier.shape = NA, alpha = .5, width = .1, colour = "black") + 
  geom_point(data = roi_sum, aes(x = roi, y = contrast), position = position_nudge(.3), colour = "BLACK")+
  geom_errorbar(data = roi_sum, aes(x=roi,y=contrast, ymin = contrast-ci, ymax = contrast+ci), position=position_nudge(x = .3, y = 0), width = .05)+
  #theme_classic() + 
  ylab("Contrast estimates") + 
  xlab("Region-of-interest") + 
  scale_fill_brewer(palette = "Greens") +
  scale_colour_brewer(palette = "Greens") + 
  ggtitle(paste("Theory-of-Mind network contrasts estimates across contrasts and regions")) + 
  theme(legend.position="none") +
  facet_wrap(~comparison) 
FS2

ggsave("experiment1/figures/S7.TIFF", plot = FS2, width = 24.7, height = 15.24, units = "cm", dpi = 300)

```

```{r}
DF.hand <- left_join(DF.hand, DF.idaq, by = c("sub","dataset"), keep = FALSE) %>%
  pivot_wider(names_from=scale, values_from = score) #
```

Center the variables for the formal analysis:
```{r}
DF.hand$cent_IDAQNA <- scale(DF.hand$IDAQNA, scale = TRUE)
DF.hand$cent_IDAQ <- scale(DF.hand$IDAQ, scale = TRUE)
DF.hand <- DF.hand %>% group_by(roi, comparison) %>% mutate(cent_contrast = scale(contrast, scale =TRUE)) #scale per roi (analyses are done per roi)
DF.hand$cent_IDAQ2 <- DF.hand$cent_IDAQ^2

```

```{r}
FS4 <- DF.hand %>%
  filter(comparison == "mental_control" | comparison == "pain_control") %>% 
  ggplot(aes(x=cent_IDAQ, y=cent_contrast)) +
  geom_point(alpha=0.5,show.legend = FALSE) +
  geom_smooth(method="lm", formula=y ~ x, se=TRUE, show.legend = FALSE, colour="#0072B2") +
  geom_smooth(method="lm", formula=y ~ x+ I(x^2), se=TRUE, show.legend=FALSE, colour = "#D55E00") +
  ...
  facet_wrap(comparison~roi, nrow = 2)
FS4

ggsave("experiment1/figures/S9.TIFF", plot = FS4, width = 24.7, height = 15.24, units = "cm", dpi = 300)
...
Mental vs. control:
```{r}
mentalC <- brm(formula = cent_contrast ~ cent_IDAQ + cent_IDAQ2,
      data = DF.hand %>% filter(comparison == "mental_control"),
      family = gaussian,
      chains = 4,
      iter = 4000,
      seed = 42, #so the model is reproducible
      file = "experiment1/models/control/mental_control.RDS")
```

Pain vs. control
```{r}
painC <- brm(formula = cent_contrast ~ cent_IDAQ + cent_IDAQ2,
      data = DF.hand %>% filter(comparison == "pain_control"),
      family = gaussian,
      chains = 4,
      iter = 4000,
      seed = 42, #so the model is reproducible
      file = "experiment1/models/control/pain_control.RDS")
```

Social vs. control
```{r}
socialC <- brm(formula = cent_contrast ~ cent_IDAQ + cent_IDAQ2,
      data = DF.hand %>% filter(comparison == "social_control"),
      family = gaussian,
      chains = 4,
      iter = 4000,
      seed = 42, #so the model is reproducible
      file = "experiment1/models/control/social_control.RDS")
```

All vs. control
```{r}
allC <- brm(formula = cent_contrast ~ cent_IDAQ + cent_IDAQ2,
      data = DF.hand %>% filter(comparison == "all_control"),
      family = gaussian,
      chains = 4,
      iter = 4000,
      seed = 42, #so the model is reproducible
      file = "experiment1/models/control/all_control.RDS")
```


```{r}
tab_model(
  mentalC, painC, socialC, allC,
  show.se = FALSE,
  #collapse.se = TRUE,
  pred.labels = c("Intercept", "linear predictor", "quadratic predictor"),
  dv.labels = c("mental events","pain events", "social events", "all events"),
  show.obs=FALSE
)
```
...
```{r}
tom_handcoded_all <- bind_rows("all" = as_tibble(as.mcmc(mentalC,  pars = c("b_cent_IDAQ", "b_cent_IDAQ2"), combine_chains = TRUE), .id = "network")  %>%
                        dplyr::rename(linear = b_cent_IDAQ, quadratic = b_cent_IDAQ2) %>%
                        pivot_longer(c("linear", "quadratic"),names_to = "varIDAQ") %>%
                        mutate(network = "Partly Cloudy: hand-coded events"))
```
...
Individual regions:
```{r}
reg_model2 <- function(region, comp){
  brm(formula = cent_contrast ~ cent_IDAQ + cent_IDAQ2,
      data = DF.hand %>% filter(roi == region & comparison == comp),
      family = gaussian,
      chains = 4,
      iter = 4000,
      seed = 42, #so the model is reproducible
      file = paste0("experiment1/models/control/", region, "_", comp, ".RDS", sep = ""))
}
```

Mental vs. control
```{r}
rtpj_mentalC <- reg_model2("rtpj", "mental_control")
ltpj_mentalC <- reg_model2("ltpj", "mental_control")
prec_mentalC <- reg_model2("prec", "mental_control")
vmpfc_mentalC <- reg_model2("vmpfc", "mental_control")
mmpfc_mentalC <- reg_model2("mmpfc", "mental_control")
dmpfc_mentalC <- reg_model2("dmpfc", "mental_control")
```

```{r}
tab_model(
  rtpj_mentalC, ltpj_mentalC, prec_mentalC, vmpfc_mentalC, mmpfc_mentalC, dmpfc_mentalC,
  show.se = TRUE,
  #collapse.se = TRUE,
  pred.labels = c("Intercept", "linear predictor", "quadratic predictor"),
  dv.labels = c("rtpj","ltpj", "prec", "vmpfc" , "mmpfc", "dmpfc"),
...
```{r}
mentalC_combined <- bind_rows("rtpj" = as_tibble(as.mcmc(rtpj_mentalC,  pars = c("b_cent_IDAQ", "b_cent_IDAQ2"), combine_chains = TRUE)),
                          "ltpj" = as_tibble(as.mcmc(ltpj_mentalC, pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),combine_chains = TRUE)),
                          "prec" = as_tibble(as.mcmc(prec_mentalC, pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),combine_chains = TRUE)),
                          "vmpfc" = as_tibble(as.mcmc(vmpfc_mentalC, pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),combine_chains = TRUE)),
                          "mmpfc" = as_tibble(as.mcmc(mmpfc_mentalC, pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),combine_chains = TRUE)),
                          "dmpfc" = as_tibble(as.mcmc(dmpfc_mentalC, pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),combine_chains = TRUE)),
                          .id = "roi")

mentalC_combined <- mentalC_combined %>%
  dplyr::rename(linear = b_cent_IDAQ, quadratic = b_cent_IDAQ2) %>%
  pivot_longer(c("linear", "quadratic"),names_to = "varIDAQ") %>%
  mutate(model = "mental vs control") %>% 
  mutate(roi = fct_relevel(roi, order[1:6])) 
```
...
Pain vs. control
```{r}
rtpj_painC <- reg_model2("rtpj", "pain_control")
ltpj_painC <- reg_model2("ltpj", "pain_control")
prec_painC <- reg_model2("prec", "pain_control")
vmpfc_painC <- reg_model2("vmpfc", "pain_control")
mmpfc_painC <- reg_model2("mmpfc", "pain_control")
dmpfc_painC <- reg_model2("dmpfc", "pain_control")
```

```{r}
tab_model(
  rtpj_painC, ltpj_painC, prec_painC, vmpfc_painC, mmpfc_painC, dmpfc_painC,
  show.se = TRUE,
  #collapse.se = TRUE,
  pred.labels = c("Intercept", "linear predictor", "quadratic predictor"),
  dv.labels = c("rtpj","ltpj", "prec", "vmpfc" , "mmpfc", "dmpfc"),
...
```{r}
painC_combined <- bind_rows("rtpj" = as_tibble(as.mcmc(rtpj_painC,  pars = c("b_cent_IDAQ", "b_cent_IDAQ2"), combine_chains = TRUE)),
                          "ltpj" = as_tibble(as.mcmc(ltpj_painC, pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),combine_chains = TRUE)),
                          "prec" = as_tibble(as.mcmc(prec_painC, pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),combine_chains = TRUE)),
                          "vmpfc" = as_tibble(as.mcmc(vmpfc_painC, pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),combine_chains = TRUE)),
                          "mmpfc" = as_tibble(as.mcmc(mmpfc_painC, pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),combine_chains = TRUE)),
                          "dmpfc" = as_tibble(as.mcmc(dmpfc_painC, pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),combine_chains = TRUE)),
                          .id = "roi")

s11 <- painC_combined %>%
  dplyr::rename(linear = b_cent_IDAQ, quadratic = b_cent_IDAQ2) %>%
  pivot_longer(c("linear", "quadratic"),names_to = "varIDAQ") %>%
  mutate(model = "pain vs control") %>% 
  mutate(roi = fct_relevel(roi, order[1:6])) %>% 
  bind_rows(mentalC_combined) %>% 
  bind_rows(tom_combined) %>% 
  mutate_at(vars(model), replace_na, "mental vs pain") %>%
  mutate_at(vars(-value),as.factor) %>%
  dplyr::rename(predictor = varIDAQ) %>%
  ggplot(., aes(value, fill = model)) +
  geom_density(alpha = .5)+
  geom_vline(xintercept = 0, color = "black", linetype = 2) +
  ggtitle(paste("Posterior distributions for the mental vs pain and mental/pain vs control models")) + 
  facet_wrap(predictor~roi, ncol =3) +
  coord_fixed(ratio = 1/40) +
  theme_classic(base_size = 8) +
  theme(strip.background = element_blank()) +
  scale_fill_manual(values=c("#0072B2", "#D55E00", "seagreen3"))+ 
  theme(axis.text.y = element_blank())

s11

ggsave("experiment1/figures/S11.TIFF", plot = s11, width = 24.7, height = 15.24, units = "cm", dpi = 300)
```


##11.2 Additional ROIs
```{r}
DF.d1 <- roi_extract(1, 101, 129, "other", 2)
DF.d2.a <- roi_extract(2, 201, 202, "other/201_202", 2)
DF.d2.b <- roi_extract(2, 203, 235, "other", 2)
DF.d3 <- roi_extract(3, 301, 322, "other", 2)
DF.d4 <- roi_extract(4, 401, 422, "other", 2)

DF.lit <- bind_rows(DF.d1, DF.d2.a, DF.d2.b, DF.d3, DF.d4) %>%
  mutate_at(vars(-contrast),as.factor)
```

Plot
```{r}
roi_sum <- summarySEwithin(DF.lit, measurevar="contrast", betweenvars="dataset", withinvars= c("roi","network"), idvar="sub") 
...
```{r}
DF.lit <- left_join(DF.lit, DF.idaq, by = c("sub","dataset"), keep = FALSE) %>%
  pivot_wider(names_from=scale, values_from = score) #
```

Center the variables for the formal analysis:
```{r}
DF.lit$cent_IDAQNA <- scale(DF.lit$IDAQNA, scale = TRUE)
DF.lit$cent_IDAQ <- scale(DF.lit$IDAQ, scale = TRUE)
DF.lit <- DF.lit %>% group_by(roi) %>% mutate(cent_contrast = scale(contrast, scale =TRUE)) #scale per roi (analyses are done per roi)
DF.lit$cent_IDAQ2 <- DF.lit$cent_IDAQ^2
...
```{r}
lit_ltpj <-  brm(formula = cent_contrast ~ cent_IDAQ + cent_IDAQ2,
      data = DF.lit %>% filter(roi == "ltpj"),
      family = gaussian,
      chains = 4,
      iter = 4000,
      seed = 42, #so the model is reproducible
...
lit_mpfc_acc <-  brm(formula = cent_contrast ~ cent_IDAQ + cent_IDAQ2,
      data = DF.lit %>% filter(roi == "rmpfc-acc"),
      family = gaussian,
      chains = 4,
      iter = 4000,
      seed = 42, #so the model is reproducible
      file = "lit-rmpfc-acc.RDS")
```

```{r}
summary(lit_ltpj)
summary(lit_mpfc_acc)
```

```{r}
tab_model(
  lit_ltpj,lit_mpfc_acc,
  show.se = FALSE,
  #collapse.se = TRUE,
  pred.labels = c("Intercept", "linear predictor", "quadratic predictor"),
  dv.labels = c("ltpj", "mpfc-acc"),
  show.obs=FALSE
)
```
...
```{r}
lit <-
  bind_rows("lit_ltpj" = as_tibble(as.mcmc(lit_ltpj,
  pars = c( "b_cent_IDAQ","b_cent_IDAQ2"),
  combine_chains = TRUE)),
  "lit_mpfc_acc" = as_tibble(as.mcmc(lit_mpfc_acc,
  pars = c( "b_cent_IDAQ","b_cent_IDAQ2"),
  combine_chains = TRUE)),
  .id = "region")
...
## 11.3 Multi-level model
ToM
```{r}
tom_ML <- update(tom, 
                      newdata = DF.roi %>% filter(network == "tom"), 
                      formula. = ~ . + (1|dataset) + (1|sub),
                      control = list(adapt_delta = 0.99, max_treedepth = 11),
                      seed = 41)

tom_ML <- update(tom_ML, formula. = ~ . - (1|sub), seed = 41)


tom_ML_p <- bind_rows("all" = as_tibble(as.mcmc(tom_ML,  pars = c("b_cent_IDAQ", "b_cent_IDAQ2"), combine_chains = TRUE), .id = "network")  %>%
                        dplyr::rename(linear = b_cent_IDAQ, quadratic = b_cent_IDAQ2) %>%
                        pivot_longer(c("linear", "quadratic"),names_to = "varIDAQ") %>%
                        mutate(network = "tom", model = "ML"))
tom_ML_p %>% 
  bind_rows(tom_all) %>% 
  mutate_at(vars(model), replace_na, "original") %>%
  mutate_at(vars(-value),as.factor) %>%
  ...
  geom_density(alpha = .5)+
  geom_vline(xintercept = 0, color = "black", linetype = 2) +
  ggtitle(paste("Posterior distributions for the original and multi-level models")) + 
...
For all regions individually 
```{r}
update_ML <- function(model_old, region){
  model_new <- update(model_old, 
                      newdata = DF.roi %>% filter(roi == region), 
                      formula. = ~ . + (1|dataset) + (1|sub),
                      control = list(adapt_delta = 0.99, max_treedepth = 11),
                      seed = 41)
}
...
Plot the difference per predictor and ToM region:
```{r}
rtpj_ML <- update_ML(rtpj, "rtpj")
rtpj_ML <- update(rtpj_ML, formula. = ~ . - (1|sub), seed = 41)

ltpj_ML <- update_ML(ltpj, "ltpj")
ltpj_ML <- update(ltpj_ML, formula. = ~ . - (1|sub), seed = 41)

prec_ML <- update_ML(prec, "prec")
prec_ML <- update(prec_ML, formula. = ~ . - (1|sub), seed = 41)

vmpfc_ML <- update_ML(vmpfc, "vmpfc")
vmpfc_ML <- update(vmpfc_ML, formula. = ~ . - (1|sub), seed = 41)

mmpfc_ML <- update_ML(mmpfc, "mmpfc")
mmpfc_ML <- update(mmpfc_ML, formula. = ~ . - (1|sub), seed = 41)

dmpfc_ML <- update_ML(dmpfc, "dmpfc")
dmpfc_ML <- update(dmpfc_ML, formula. = ~ . - (1|sub), seed = 41)

tom_combined_ML <- bind_rows("rtpj" = as_tibble(as.mcmc(rtpj_ML,  pars = c("b_cent_IDAQ", "b_cent_IDAQ2"), combine_chains = TRUE)),
                          "ltpj" = as_tibble(as.mcmc(ltpj_ML, pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),combine_chains = TRUE)),
                          "prec" = as_tibble(as.mcmc(prec_ML, pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),combine_chains = TRUE)),
                          "vmpfc" = as_tibble(as.mcmc(vmpfc_ML, pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),combine_chains = TRUE)),
                          "mmpfc" = as_tibble(as.mcmc(mmpfc_ML, pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),combine_chains = TRUE)),
                          "dmpfc" = as_tibble(as.mcmc(dmpfc_ML, pars = c("b_cent_IDAQ", "b_cent_IDAQ2"),combine_chains = TRUE)),
                          .id = "roi")
...
```{r}
ltpj_ml <- brm(formula = cent_contrast ~ cent_IDAQ + cent_IDAQ2 + (1|dataset),
      data = DF.roi %>% filter(roi == "ltpj"),
      family = gaussian,
      chains = 4,
      iter = 4000,
      control = list(adapt_delta = 0.99, max_treedepth = 10),
      seed = 42, #so the model is reproducible
      file = "experiment1/models/control/ltpj_ml.RDS")
```
```{r}
summary(ltpj_ml)
```

## 11.4 False belief and IDAQ
This is data from the validation analysis reported in the methods of experiment 2. To provide further validation and justification for this task as a measure of general Theory-of-Mind, we re-analysed data from two studies (Darda et al., 2018, Darda & Ramsey, in prep). Participants (n = 86) completed the False Belief task to localise the Theory-of-Mind network while behavioural indices (accuracy and reaction times) were recorded. Twenty of the 86 participants also completed the IDAQ. 

```{r}
DF.falsebelief <- read.csv("experiment2/data/exploratory/falsebelief_idaq.csv") %>% 
  pivot_longer(cols = c(dmpfc:vmpfc), names_to = "roi", values_to = "contrast") %>% 
  mutate(cent_IDAQ = scale(idaq, scale = TRUE)) %>% 
  group_by(roi) %>% 
  mutate(cent_contrast = scale(contrast, scale =TRUE),
         cent_IDAQ2 = cent_IDAQ^2
  )  
```
...
F1E <- DF.falsebelief %>%
  ...
  labs(
    x="Dispositional anthropomorphism",
    y="Theory-of-Mind network activity during False Belief task"
  ) #+ theme_classic()

F1E 

ggsave("experiment1/figures/F1E.TIFF", plot = F1E, width = 24.7, height = 15.24, units = "cm", dpi = 300)
```

```{r}
tom_fc <- brm(formula = cent_contrast ~ cent_IDAQ + cent_IDAQ2,
      data = DF.falsebelief,
      family = gaussian,
      chains = 4,
      iter = 4000,
      seed = 42, #so the model is reproducible
      file = "experiment1/models/regions/tom_fb.RDS")
```
...
tab_model(
  ...
  #collapse.se = TRUE,
  pred.labels = c("Intercept", "linear predictor", "quadratic predictor"),
  dv.labels = "tom during false belief",
  show.obs=FALSE
)
```

Compare it to the Partly Cloudy model (reverse correlation and hand-coded events)
```{r}
tom_fb_all <- bind_rows("all" = as_tibble(as.mcmc(tom_fc,  pars = c("b_cent_IDAQ", "b_cent_IDAQ2"), combine_chains = TRUE), .id = "network")  %>%
                        dplyr::rename(linear = b_cent_IDAQ, quadratic = b_cent_IDAQ2) %>%
                        pivot_longer(c("linear", "quadratic"),names_to = "varIDAQ") %>%
                        mutate(network = "False Belief"))

F1F <- tom_all %>%
  mutate(network = "Partly Cloudy: reverse correlation events") %>% 
  bind_rows(., tom_fb_all, tom_handcoded_all) %>%
  mutate_at(vars(-value),as.factor,) %>%
  dplyr::rename(predictor = varIDAQ, task = network) %>%
  ggplot(., aes(value, fill = task)) +
  geom_density(alpha = .5)+
  geom_vline(xintercept = 0, color = "black", linetype = 2) +
  ggtitle(paste("Anthropomorphism and Theory-of-Mind network activity")) + 
  ...
  theme(strip.background = element_blank())
F1F
ggsave("experiment1/figures/F1F.TIFF", plot = F1F, width = 24.7, height = 15.24, units = "cm", dpi = 300)
