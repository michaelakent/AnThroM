---
title: "Testing the relation between dispositional anthropomorphism and Theory-of-Mind network activation"
...
```{r}
#load data
DF.dataset1 <- read_tsv(file = "dataset_1/participants.tsv")
DF.dataset2 <- read_tsv(file = "dataset_2/participants.tsv")
DF.dataset3 <- read_tsv(file = "dataset_3/participants.tsv")
DF.dataset4 <- read_tsv(file = "dataset_4/participants.tsv")
...
```{r}
#load data
DF.dataset1  <- read_tsv(file = paste("dataset_1/derivatives/mriqc/group_", modality, ".tsv", sep ="")) %>%
  gather("measure", "value", 2:46) %>%
  select("bids_name","measure", "value")

DF.dataset2 <- read_tsv(file = paste("dataset_2/derivatives/mriqc/group_", modality, ".tsv", sep ="")) %>%
  gather("measure", "value", 2:46) %>%
  select("bids_name","measure", "value")

DF.dataset3 <- read_tsv(file = paste("dataset_3/derivatives/mriqc/group_", modality, ".tsv", sep ="")) %>%
  gather("measure", "value", 2:46) %>%
  select("bids_name","measure", "value")

DF.dataset4 <- read_tsv(file = paste("dataset_4/derivatives/mriqc/group_", modality, ".tsv", sep ="")) %>%
  gather("measure", "value", 2:46) %>%
  select("bids_name","measure", "value") 
...
```{r}
#load data 
DF.d1  <- read_csv(file = "dataset_1/derivatives/IDAQ_dataset1.csv") %>%
  gather("sub", "value", 4:32)

DF.d2  <- read_csv(file = "dataset_2/derivatives/IDAQ_dataset2.csv") %>%
  gather("sub", "value", 4:38) 

DF.d3  <- read_csv(file = "dataset_3/derivatives/IDAQ_dataset3.csv") %>%
  gather("sub", "value", 4:25) 

DF.d4  <- read_csv(file = "dataset_4/derivatives/IDAQ_dataset4.csv") %>%
  gather("sub", "value", 4:25) 
...
Run the analysis for the anthropomorphism subscale:
```{r}
DF.test = DF.idaq %>% 
  filter(scale == "IDAQ") %>% 
...
ord.1 <- brm(
  ...
  file = 'models/control/ord.1~simple.RDS'
) 
```
...
ord.2 <- brm(
  ...
  data  = DF.test,  
  family  = cumulative("logit"),
  file = 'models/control/ord.2~random.RDS'
) 
```
...
ord.3 <- brm(
  ...
  data  = DF.test,  
  family = acat("logit"),
  file = 'models/control/ord.3~category.RDS'
)  
```
...
ord.4 <- brm(
  ...
  data  = DF.test,  
  family = acat("logit"),
  file = 'models/control/ord.4~category2.RDS'
) 
```
...
ord.5 <- brm(
  ...
  data = DF.test,
  family  = cumulative("logit"),
  file = 'models/control/ord.5~control.RDS'
) 
```
...
#save(modelC1, file="models/control/modelC1.RData")
load('modelC1.RData')
modelC1 
...
ord.2E <- brm(
  ...
  file = 'models/control/ord.2E~random.RDS'
) 
```
...
9. Model 3 while excluding these participants (does not convergence): 
```{r}
ord.3E <- brm(
  value ~ 1 + cs(dataset) +
    (cs(1)|sub) + (cs(1)|itemnr),
  ...
  file = 'models/control/ord.3E~category.RDS'
)  
...
FS1 <- DF.idaq %>%
  ...
  facet_wrap(~scale)
FS1
ggsave("figures/S1.TIFF", plot = FS1, width = 24.7, height = 15.24, units = "cm", dpi = 300)
...
F1A
ggsave("figures/F1A.TIFF", plot = F1A, width = 24.7, height = 15.24, units = "cm", dpi = 300)
```
...
ggsave("figures/S2.TIFF", plot = FS2, width = 24.7, height = 15.24, units = "cm", dpi = 300)
```
...
ggsave("figures/F1C.TIFF", plot = F1C, width = 24.7, height = 15.24, units = "cm", dpi = 300)
```
...
tom.compare <- brm(
  ...
  #family  = cumulative("logit"),
  file = 'models/control/tom.compare.RDS'
) 
...
FS3 <- DF.roi %>%
  ...
  facet_wrap(~dataset)
FS3
ggsave("figures/S3.TIFF", plot = FS3, width = 24.7, height = 15.24, units = "cm", dpi = 300)
```
...
pain.compare <- brm(
  ...
  #family  = cumulative("logit"),
  file = 'models/control/pain.compare.RDS'
) 
...
ggsave("figures/F1D.TIFF", plot = F1D, width = 24.7, height = 15.24, units = "cm", dpi = 300)
```
...
ggsave("figures/F1E.TIFF", plot = F1E, width = 24.7, height = 15.24, units = "cm", dpi = 300)
...
reg_model <- function(region){
  brm(formula = cent_contrast ~ cent_IDAQ + cent_IDAQ2,
      ...
      file = paste0("models/regions/", region, ".RDS", sep = ""))
}
```
...
ggsave("figures/F2.TIFF", plot = F2, width = 24.7, height = 15.24, units = "cm", dpi = 300)
```
...
ggsave("figures/S4.TIFF", plot = S4, width = 24.7, height = 15.24, units = "cm", dpi = 300)
```
...
ggsave("figures/S5.TIFF", plot = S5, width = 24.7, height = 15.24, units = "cm", dpi = 300)
```
...
ggsave("figures/S6.TIFF", plot = S6, width = 24.7, height = 15.24, units = "cm", dpi = 300)
```
...
```{r}
rtpj_bias <- update_model(rtpj, "rtpj")
ltpj_bias <- update_model(ltpj, "ltpj")
prec_bias <- update_model(prec, "prec")
vmpfc_bias <- update_model(vmpfc, "vmpfc")
mmpfc_bias <- update_model(mmpfc, "mmpfc")
dmpfc_bias <- update_model(dmpfc, "dmpfc")
```
