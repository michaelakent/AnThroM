...
## 10.1 Update effectance model to excluded participants who have seen the film

```{r}
modelef_seen <- update(modelef, newdata = DF.exp2 %>% filter(seen == 2), seed = 41)
summary(modelef_seen)
hdi_rope(modelef_seen) 
```

## 10.2 Update false belief model to exclude participants who have seen the film

```{r}
modelfb_seen <- update(modelfb, newdata = DF.exp2 %>% filter(seen == 2), seed = 41)
summary(modelfb_seen)
hdi_rope(modelfb_seen) 
```

## 10.3 Update effectance model without excluding participants 
Don't run section 4.5.
```{r}
modelef_inc <- update(modelef, newdata = DF.exp2, seed = 41)
summary(modelef_inc)
hdi_rope(modelef_inc) 
```

## 10.4 Update false belief model without excluding participants 
Don't run section 4.5.
```{r}
modelfb_inc <- update(modelfb, newdata = DF.exp2, seed = 41)
summary(modelfb_inc)
hdi_rope(modelfb_inc) 
```
