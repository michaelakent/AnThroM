...
p1 <- DF.rating %>%
  mutate(event_nr = factor(event_nr, levels=c(1:16), ordered=TRUE)) %>% 
  dplyr::group_by(prolific_id, event_nr, measure) %>% #focus and event_type are included or excluded based on Model C1/C2
  dplyr::summarise(rating = mean(as.numeric(response))) %>% 
  ggplot(., aes(event_nr, rating)) +
  geom_line(aes(group = prolific_id), alpha=0.2) +
...
```{r, echo=FALSE, warning=FALSE}
DF.mean <- summarySEwithin(DF.rating, measurevar="response", withinvars= c("event_type", "focus","measure"), idvar="prolific_id")

p2 <- DF.rating %>%
  dplyr::group_by(prolific_id, event_type, focus, measure) %>% #focus and event_type are included or excluded based on Model C1/C2
  dplyr::summarise(rating = mean(as.numeric(response))) %>% 
...
```{r, echo=FALSE}
DF.mean <- summarySEwithin(DF.idaq, measurevar="score", withinvars= "Scale", idvar="prolific_id")
...
```{r, echo=FALSE}
DF.mean <- summarySEwithin(DF.fa, measurevar="acc", withinvars= "p_b", idvar="prolific_id")
...
# 6. Title {.tabset .tabset-fade .tabset-pills}
...
## 8.4 Model 1 - anthropomorphism and effectance
...
## 8.5 Model 2 - anthropomorphism and false belief
