...
ctR <- DF.rating %>%
  ...
  #annotate("text", x = 0.75, y = 1.15, label = "Preregistered exclusion criteria") +
...
# 5. Visualise data {.tabset .tabset-fade .tabset-pills}

## 5.1 Rating task

```{r}
rating_summary <- summarySEwithin(DF.rating, measurevar="response", withinvars= c("focus","measure", "event_typeM"), idvar="prolific_id")


DF.rating %>%
  pgroup_by(prolific_id, measure, focus, event_typeM) %>%
  ...
  geom_point(data = rating_summary, aes(x = event_typeM, y = rating), position = position_nudge(.3), colour = "BLACK")+
