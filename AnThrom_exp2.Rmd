...
---
#### Project AnThroM: experiment 2
Exploring the relationship between anthropomorphism and Theory-of-Mind in brain and behaviour: Experiment 2 behaviour
June 2019 - November 2020

# 1. Details {.tabset .tabset-fade .tabset-pills}

## 1.1 Preregistration
The preregistration can be found here: https://osf.io/tuq4a
...
The data can be downloaded from the [OSF project page](https://osf.io/a8pyr/). Data is anonymised using the [anolific script](https://github.com/rhortensius/anolific). 
...
DF.exp2 <- list.files("experiment2/data/main",pattern="\\PARTICIPANT*", full.names=TRUE) %>% 
  map_dfr(read_csv) %>% 
  #rename(participant_id = prolific_id) %>% #already renamed using anolific script: https://github.com/rhortensius/anolific
...
The following participant contacted us and indicated they pressed enter by mistake (there is no moving back option), we will not exclude this participant for not providing an answer (and 'failing' the attention check):
    ...
    ead376fb11cc52e3a7637c428bd2875f0178adb5f50638150d80ab9f5ac28046

```{r}
excluded_participants <- DF.prediction %>% 
  filter(is.na(prediction) & participant_id != "ead376fb11cc52e3a7637c428bd2875f0178adb5f50638150d80ab9f5ac28046") %>% 
...
Calculate reliability anthropomorphism:
```{r, eval=FALSE}
DF.rating %>% 
  filter(measure == "anthropomorphism") %>%
  group_by(participant_id)%>%
  mutate(ItemNr = c(1:length(response)))  %>%
  select(-measure,-event_type, -rt, -focus, -item, -eventnr)  %>%
  pivot_wider(names_from = ItemNr, values_from = response) %>%
  select(-participant_id) %>%
  psych::alpha(na.rm = TRUE) 

alpha.ci(0.9838,145,241,p.val=.05,digits=5)
```

Calculate reliability effectance:
```{r, eval=FALSE}
DF.rating %>% 
  filter(measure == "effectance") %>%
  group_by(participant_id)%>%
  mutate(ItemNr = c(1:length(response)))  %>%
  select(-measure,-event_type, -rt, -focus, -item, -eventnr)  %>%
  pivot_wider(names_from = ItemNr, values_from = response) %>%
...
alpha.ci(0.9626,145,241,p.val=.05,digits=5)
```
...
```{r}
##for control (revision)
DF.ratingC <- DF.rating 
...
Plot for manuscript without .5 exclusion criteria (don't run 4.4):
```{r}
DF.mean <- summarySEwithin(DF.fa, measurevar="acc", withinvars= "p_b", idvar="participant_id") %>% 
  filter(p_b == "b") 
theme_set(theme_classic(base_size = 20)) 
...
ggscatterstats(
  ...
  x = cent_sitant,
  y = cent_disant,
  title = "Situational and dispositional anthropomorphism"
)
```
...
p6 <- DF.exp2 %>%
  ...
  labs(
    x="False Belief accuracy",
    y="Effectance rating")
p6

patched <- p5 | p6

ggsave("experiment2/figures/S11.TIFF", plot = patched, width = 12, height = 5.08, units = "cm", dpi = 300)
...
ggscatterstats(
  ...
  x = cent_falsebelief,
  y = cent_effectance,
  title = "False Belief accuracy and effectance ratings"
)
```
...
p11
ggsave("experiment2/figures/S12.TIFF", plot = p11, height = 5.08, width = 12, units = "cm", dpi = 300)
```
...
modelef <- brm(formula = cent_effectance ~ cent_sitant + cent_sitant2 + cent_disant + cent_disant2, 
      ...
      iter = 4000,
      seed = 42, 
      file = "experiment2/models/modelef2.RDS")
```
...
modelfb <- brm(formula = cent_falsebelief ~ cent_sitant + cent_sitant2 + cent_disant + cent_disant2, 
      ...
      iter = 4000,
      seed = 42, 
      file = "experiment2/models/modelfb2.RDS")
```
...
hdi_rope <- function(model){
  ...
  plot(rope_model) + theme_classic() + 
    ...
    ggtitle("DV: False belief accuracy") +
    scale_y_discrete(labels = c("Situational (quadratic)", "Situational (linear)", "Dispositional (quadratic)", "Dispositional (linear)")) +
  #theme(legend.position="none") +
      scale_x_continuous(limits=c(-0.2, 0.6), breaks=c(-0.2,0, 0.2))
}
```
...
combined <- combined_models %>%
  ...
  mutate(Decision= ifelse(model=="modelef"&var_anth=="b_cent_sitant", "Reject",
                          ...
                          ifelse(model=="modelfb"&var_anth=="b_cent_sitant2", "Accept",
                          ifelse(model=="modelfb"&var_anth=="b_cent_disant2", "Undecided",
                          ifelse(model=="modelef"&var_anth=="b_cent_disant2","Undecided",
                          ifelse(model=="modelef"&var_anth=="b_cent_sitant2", "Accept", "NA"))))))) %>%
  group_by("Decision")%>%
...
ggsave("experiment2/figures/HDIROPE.TIFF", width = 24,height = 16, plot = combined, units = "cm", dpi = 300)
```

# 10. Control models {.tabset .tabset-fade .tabset-pills}

## 10.1 Update the full model to excluded participants who have seen the film
```{r}
model_f_seen <- update(model_f, newdata = DF.exp2 %>% filter(seen == 2), seed = 41)
summary(model_f_seen)
```

## 10.2 Posterior plots - first viewing
```{r,eval=FALSE}
tom_combined_seen <- as_tibble(as.mcmc(model_f_seen, combine_chains = TRUE)) %>% 
  pivot_longer(cols = ends_with(c("ant", "ant2")), names_to = "varAnthro") %>% 
  select(varAnthro, value) %>% 
  separate(varAnthro, c("dv", "predictor"), sep = "_cent_")
```
...
Plot the distributions:
```{r,eval=FALSE}
p13 <- tom_combined_seen %>%
  mutate_at(vars(-value),as.factor) %>%
  bind_rows(tom_combined, .id = "model") %>% 
  mutate(model= ifelse(model == 1,"first viewing","original")) %>% 
  mutate(dv = ifelse(dv == "b_centfalsebelief", "false belief accuracy", "effectance rating")) %>% 
  ggplot(., aes(value, fill = predictor, by = predictor)) +
  geom_density(alpha = .5)+
  geom_vline(xintercept = 0, color = "black", linetype = 2) +
  ggtitle(paste("Posterior distributions for the original and first viewing only samples")) + 
  facet_wrap(model~dv) +
  theme(strip.background = element_blank()) +
  scale_fill_manual(values=c("#0072B2", "#000033", "#D55E00", "#660000")) 
p13

ggsave("experiment2/figures/S13.TIFF", plot = p13, width = 30, height = 20, units = "cm", dpi = 300)
```

## 10.3 Update full model without excluding participants 
Don't run section 4.5. 10.1 and 10.2 (eval=FALSE).
```{r, eval=FALSE}
model_f_inc <- update(model_f, newdata = DF.exp2, seed = 41)
summary(model_f_inc)
```
## 10.4 Posterior plots - no exclusion 
```{r}
tom_combined_full <- as_tibble(as.mcmc(model_f_inc, combine_chains = TRUE)) %>% 
  pivot_longer(cols = ends_with(c("ant", "ant2")), names_to = "varAnthro") %>% 
  select(varAnthro, value) %>% 
  separate(varAnthro, c("dv", "predictor"), sep = "_cent_")
```
...
Plot the distributions:
```{r}
p14 <- tom_combined_full %>%
  mutate_at(vars(-value),as.factor) %>%
  bind_rows(tom_combined, .id = "model") %>% 
  mutate(model= ifelse(model == 1,"without","with")) %>% 
  mutate(dv = ifelse(dv == "b_centfalsebelief", "false belief accuracy", "effectance rating")) %>% 
  ggplot(., aes(value, fill = predictor, by = predictor)) +
  geom_density(alpha = .5)+
  geom_vline(xintercept = 0, color = "black", linetype = 2) +
  ggtitle(paste("Posterior distributions with and without exclusion criteria applied")) + 
  facet_wrap(model~dv) +
  theme(strip.background = element_blank()) +
  scale_fill_manual(values=c("#0072B2", "#000033", "#D55E00", "#660000")) 
p14

ggsave("experiment2/figures/S14.TIFF", plot = p14, width = 30, height = 20, units = "cm", dpi = 300)
```

## 10.4 Situational anthropomorphism and effectance ratings

Stork to cloud
```{r}
DF.ratingC %>% 
  dplyr::group_by(participant_id, measure, focus) %>% 
  dplyr::summarise(rating = mean(as.numeric(response))) %>%  
  pivot_wider(names_from = c(focus,measure), values_from = rating) %>% 
  ungroup() %>% 
  ggscatterstats(
    x = cloud_anthropomorphism,
    y = stork_effectance,
    title = "cloud_anthropomorphism - stork_effectance"
  )
...
DF.ratingC %>% 
  ...
  ungroup() %>% 
  ggscatterstats(
    x = stork_anthropomorphism,
    y = cloud_effectance,
    title = "stork_anthropomorphism - cloud_effectance"
  )
```

Time 1 --> Time 2
```{r}
DF.ratingCtime<-DF.ratingC %>% 
  dplyr::group_by(participant_id, measure, eventnr) %>% 
  dplyr::summarise(rating = mean(as.numeric(response))) %>%  
  pivot_wider(names_from = measure, values_from = rating) 

ccf(DF.ratingCtime$anthropomorphism, DF.ratingCtime$effectance, lag.max = 16, plot = TRUE, na.action = na.pass, main = "lagged correlation between anthropomorphism and effectance")

acf(DF.ratingCtime$anthropomorphism, lag.max = 16, plot = TRUE, na.action = na.pass, main = "lagged correlation  anthropomorphism")

acf(DF.ratingCtime$effectance, lag.max = 16, plot = TRUE, na.action = na.pass, main = "lagged correlation  effectance")
```

# 11. Exploratory models {.tabset .tabset-fade .tabset-pills}

## 11.1 Load the data 

After 4:02 minutes the film was paused and the participants were  asked to describe in one or two sentences what they think would happen next (to a maximum of 2000 characters including spaces). The code below explores these predictions and relationship with anthropomorphism. 

Authors RH and MK rated:

    - rating/accuracy: How corect was the prediction was (scale from incorrect to correct, as well as a binary decision)
    - tom: To what extent does the response relied on/contain Theory-of-Mind? (scale from very little to very much)
    
```{r}
DF.exploratory <- DF.exp2 %>% 
  left_join(read.csv(file = "experiment2/data/exploratory/prediction.csv") , by = c("participant_id", "prediction")) %>% 
  left_join(read.csv(file = "experiment2/data/exploratory/tom.csv") , by = "participant_id") %>% filter(seen == 2) %>% 
  mutate(
    cent_tom = scale(tom_combined, scale = TRUE), #tom rating
    cent_acc = scale(rating_combined, scale = TRUE)) %>% #accuracy rating
  drop_na() #one participant was included but did not provide prediction (see 4.1)
```

## 11.2 Reliability of ratings
Use ggstatsplot to inspect the reliability of  the ratings
```{r}
ggscatterstats(
  data = DF.exploratory,
  x = rating_rh,
  y = rating_mk,
  title = "Reliability of ratings between coders - accuracy"
)
...
ggscatterstats(
  ...
  x = tom_rh,
  y = tom_mk,
  title = "Reliability of ratings between coders - ToM"
)
```

## 11.3 Descriptive stats
Number of (in)correct predictions
```{r}
DF.exploratory %>% 
  dplyr::group_by(accuracy_discussed) %>%
  tally() %>%
  knitr::kable(., "html", caption = "binary prediction") %>%
  kable_styling("striped")
```

```{r}
summarySE(DF.exploratory, measurevar="rating_combined") %>%
  knitr::kable(., "html", caption = "accuracy") %>%
  kable_styling("striped")

summarySE(DF.exploratory, measurevar="tom_combined") %>%
  knitr::kable(., "html", caption = "tom") %>%
  kable_styling("striped")

summarySE(DF.exploratory, measurevar="tom_combined", groupvars= "accuracy_discussed") %>%
  knitr::kable(., "html", caption = "tom for accurary groups") %>%
  kable_styling("striped")
```
## 11.4 Exploration of indices
```{r}
ggscatterstats(
  data = DF.exploratory,
  x = rating_combined,
  y = tom_combined,
  title = "Rating of accuracy and ToM"
)
```
```{r}
ggbetweenstats(
  data = DF.exploratory,
  x = accuracy_discussed,
  y = tom_combined,
  title = "Tom for accurate/inaccurate",
  type = "Bayesian"
)
```
...
DF.exploratory %>% 
  ggcorrmat(
  cor.vars = c(5,7,19,20),
  output = "dataframe",
  digits = 2
) %>% 
  select(1:3,5:6) %>% 
  knitr::kable(., "html", caption = "Theory-of-Mind indices") %>%
  kable_styling("striped")
```
...
ggscatterstats(
  ...
  x = cent_acc,
  y = cent_effectance,
  title = "Accuracy of the open-ended prediction and effectance ratings",
)
...
ggscatterstats(
  ...
  x = cent_acc,
  y = cent_falsebelief,
  title = "Accuracy and false belief"
)
...
ggscatterstats(
  ...
  x = cent_tom,
  y = cent_effectance,
  title = "Theory-of-Mind of prediction and effectance ratings"
)
...
ggscatterstats(
  ...
  x = cent_tom,
  y = cent_falsebelief,
  title = "Theory-of-Mind of prediction and false belief performance"
)
```


## 11.5 Visualise the relationship between prediction and anthropomorphism 
Rating of accuracy of prediction
```{r} 
p15 <- DF.exploratory %>% #this code needs to be improved, just quick and dirty
  dplyr::rename(Situational = cent_sitant, 
                Dispositional = cent_disant, 
                `Theory-of-Mind reliance` = cent_tom, 
                `Accuracy of prediction` = cent_acc) %>% 
  pivot_longer(cols = c(Situational, Dispositional), names_to = "measure",  values_to = "value") %>%
  pivot_longer(cols = c(`Theory-of-Mind reliance`, `Accuracy of prediction`), names_to = "predictor",  values_to = "value_pred") %>%
  mutate(measure = fct_relevel(measure, c("Situational", "Dispositional"))) %>% 
  ggplot(aes(x=value, y=value_pred)) +
  geom_point(alpha=0.5,show.legend = FALSE) +
  geom_smooth(method="lm", formula=y ~ x, se=TRUE, show.legend = FALSE, colour="#0072B2") +
  geom_smooth(method="lm", formula=y ~ x+ I(x^2), se=TRUE, show.legend=FALSE, colour = "#D55E00") +
  coord_fixed(ratio = 1/1) +
  ...
  facet_wrap(predictor~measure)

p15
ggsave("experiment2/figures/SE.TIFF", plot = p15, width = 30, height = 20, units = "cm", dpi = 300)

```

Rating of theory-of-Mind of prediction
```{r}
p16 <- DF.exploratory %>% #can be removed
  dplyr::rename(situational = cent_sitant, dispositional = cent_disant) %>% 
  pivot_longer(cols = c(situational, dispositional), names_to = "measure",  values_to = "value") %>%
  mutate(measure = fct_relevel(measure, c("situational", "dispositional"))) %>% 
  ggplot(aes(x=value, y=cent_tom)) +
  geom_point(alpha=0.5,show.legend = FALSE) +
  geom_smooth(method="lm", formula=y ~ x, se=TRUE, show.legend = FALSE, colour="#0072B2") +
  geom_smooth(method="lm", formula=y ~ x+ I(x^2), se=TRUE, show.legend=FALSE, colour = "#D55E00") +
  coord_fixed(ratio = 1/1) +
  labs(
    x="Anthropomorphism",
    y="Theory-of-Mind of prediction") +
  facet_wrap(~measure)

#patchedE <- p15 / p16
#patchedE
#ggsave("experiment2/figures/SE.TIFF", plot = patchedE, width = 30, height = 20, units = "cm", dpi = 300)

```

## 11.6 Testing the relationship between prediction and anthropomorphism 
```{r}
p17 <- DF.exploratory %>%
  pivot_longer(cols = c("cent_acc", "cent_tom"), names_to = "dv") %>%
  ggplot(.) +
  geom_density(aes(x=value, group = dv, fill = dv), alpha = .1)
p17
```
...
Rating of accuracy of prediction
```{r}
modelacc <- brm(formula = cent_acc ~ cent_sitant + cent_sitant2 + cent_disant + cent_disant2, 
      data = DF.exploratory,
      family = skew_normal(),
      prior = priors1,
      chains = 4,
      iter = 4000,
      seed = 42, 
      file = "experiment2/models/modelacc.RDS")
```

Rating of theory-of-Mind of prediction
```{r}
modeltom <- brm(formula = cent_tom ~ cent_sitant + cent_sitant2 + cent_disant + cent_disant2, 
      data = DF.exploratory,
      family = skew_normal(),
      prior = priors1,
      chains = 4,
      iter = 4000,
      seed = 42, 
      file = "experiment2/models/modeltom.RDS")
```
...
summary(modeltom)
```
## 11.7 Posterior distribution
```{r}
tab_model(
  modelacc, modeltom,
  #show.se = TRUE, 
  pred.labels = c("Intercept", "linear situational", "quadratic situational", "linear dispositional", "quadratic dispositional"),
  dv.labels = c("Accuracy", "Theory-of-Mind"),
  show.obs=FALSE
)
```

```{r}
exp_combined <- bind_rows(
"acc" = as_tibble(as.mcmc(modelacc, pars = c(
  "b_cent_sitant","b_cent_sitant2", "b_cent_disant", "b_cent_disant2"), 
  combine_chains = TRUE)),
  "tom" = as_tibble(as.mcmc(modeltom, pars = c(
  "b_cent_sitant","b_cent_sitant2", "b_cent_disant", "b_cent_disant2"), 
  combine_chains = TRUE)), .id = "dv") %>% 
  pivot_longer(cols = ends_with(c("ant", "ant2")), names_to = "predictor")
```
...
e3 <- exp_combined %>%
  ...
  theme(strip.background = element_blank()) +
  scale_fill_discrete() +
  scale_fill_manual(values=c("#0072B2", "#000033", "#D55E00", "#660000"), name = "predictor", labels = c("Dispositional linear", "Dispositional quadratic", "Situational linear", "Situational quadratic")) 
e3

ggsave("experiment2/figures/e3.TIFF", plot = e3, width = 30, height = 20, units = "cm", dpi = 300)

```

## 11.8 Accurate vs. not accurate - ToM models:
1. Incorrect (n = 151)
```{r}
model_incorrect <- brm(formula = cent_tom ~ cent_sitant + cent_sitant2 + cent_disant + cent_disant2, 
      data = DF.exploratory %>% filter(accuracy_discussed == "incorrect"),
      family = skew_normal(),
      prior = priors1,
      chains = 4,
      iter = 4000,
      seed = 42, 
      file = "experiment2/models/model_incorrect.RDS")
```
2. Correct (n = 75)
```{r}
model_correct <- brm(formula = cent_tom ~ cent_sitant + cent_sitant2 + cent_disant + cent_disant2, 
      data = DF.exploratory %>% filter(accuracy_discussed == "correct"),
      family = skew_normal(),
      prior = priors1,
      chains = 4,
      iter = 4000,
      seed = 42, 
      file = "experiment2/models/model_correct.RDS")
```
...
```{r}
tab_model(
  model_incorrect, model_correct,
  #show.se = TRUE, 
  pred.labels = c("Intercept", "linear situational", "quadratic situational", "linear dispositional", "quadratic dispositional"),
  dv.labels = c("Incorrect", "Correct"),
  show.obs=FALSE
)
```

```{r}
prediction_combined <- bind_rows(
"incorrect" = as_tibble(as.mcmc(model_incorrect, pars = c(
  "b_cent_sitant","b_cent_sitant2", "b_cent_disant", "b_cent_disant2"), 
  combine_chains = TRUE)),
  "correct" = as_tibble(as.mcmc(model_correct, pars = c(
  "b_cent_sitant","b_cent_sitant2", "b_cent_disant", "b_cent_disant2"), 
  combine_chains = TRUE)), .id = "dv") %>% 
  pivot_longer(cols = ends_with(c("ant", "ant2")), names_to = "predictor")
```
...
ggsave("experiment2/figures/e4.TIFF", plot = e4, width = 30, height = 20, units = "cm", dpi = 300)
...
p17 <- DF.exploratory %>%
  ...
  labs(
    x="Anthropomorphism",
    y="Theory-of-Mind of prediction") +
  facet_wrap(accuracy_discussed~measure)
p17
```
