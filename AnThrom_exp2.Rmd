...
```{r}
events <- read_csv("/Users/rh/PsyCloud/Technician/1_AnThrom/Experiment_2/task/pilot/events_pilot.csv")
...
Explain it:
```{r, echo = TRUE, message=FALSE}
DF.rating <- dir_ls(paste("/Users/rh/PsyCloud/Technician/1_AnThrom/Experiment_2/data/pilot"), regexp = "\\.csv$") %>% 
...
DF.rating %>%
  filter(event_nr != "end") %>%
  group_by(sub, measure, focus, event_type) %>%
  summarise(rating = mean(response)) %>%
  ggplot(., aes(x=event_type, y=rating, fill = focus)) +
  geom_point(aes(colour = event_type), position=position_jitter(width = .05), size = 1, shape = 21, colour = "Black") +
...
DF.rating %>%
  filter(event_nr != "end") %>%
  group_by(sub, measure) %>%
  summarise(ratingSD = sd(response)) %>%
  ggplot(., aes(x=measure, y=ratingSD, fill = measure)) +
  geom_point(aes(colour = measure), position=position_jitter(width = .05), size = 1, shape = 21, colour = "Black") +
  geom_flat_violin(position=position_nudge(x = .2, y = 0),adjust =2, trim = FALSE, alpha = .75, colour = "Black") +
...
DF.rating <- dir_ls(paste("/Users/rh/PsyCloud/Technician/1_AnThrom/Experiment_2/data/pilot"), regexp = "\\.csv$") %>% 
  map_dfr(read_csv, .id = "id") %>%
  select("please type in a series of digits", "eventnr", "event_type",  "measure", "item", "focus", "key_resp_1.rt", "key_resp_2.rt", "slider.response")  %>%
  rename_at(1,~"sub",3) %>%
  mutate_at(vars(-key_resp_1.rt, -key_resp_2.rt, -slider.response),as.factor) %>%
  mutate(response = coalesce(key_resp_1.rt, key_resp_2.rt, slider.response)) %>%
  select(sub, measure, item, focus, response) %>%
  filter(!is.na(response) & !is.na(focus)) %>%
  group_by(sub) %>%
  mutate(trial_nr = c(1:length(response))) %>%
...
```{r}
fa_ca <- read_csv("/Users/rh/PsyCloud/Technician/1_AnThrom/Experiment_2/task/false_belief_correct_answers.csv")  %>%
...
```{r, echo = TRUE, message=FALSE}
DF.fa <- dir_ls(paste("/Users/rh/PsyCloud/Technician/1_AnThrom/Experiment_2/data/pilot"), regexp = "\\.csv$") %>% 
  map_dfr(read_csv, .id = "id") %>%
...
## 4.1 Calculation of individual scores:

Get the IDAQ data for all the participants:
```{r, echo = TRUE, message=FALSE}
idaq <- read_csv("/Users/rh/PsyCloud/Technician/1_AnThrom/Experiment_2/task/idaq_scale.csv") 

#load data (/Volumes/Project0255/dataset_1/sourcedata/)
DF.idaq  <- read_csv(file = "/Users/rh/PsyCloud/Technician/1_AnThrom/Experiment_2/data/IDAQ_test.csv") %>%
  pivot_longer(cols = desert_lethargic:dog_goodlooking, names_to = "item", values_to = "response") %>%
  group_by(session) %>%
  filter(!is.na(check) & !is.na(ended)) %>%
  mutate(itemnr = c(1:30)) %>%
  left_join(., idaq, by = "itemnr") %>%
  mutate(duration = (ended - created)/60) %>%
  ungroup()  %>%
  select(check, duration, itemnr, scale, response)
```

## 4.2 Data exclusion and missing responses

Participants who click through the IDAQ questionnaire, i.e. with total duration of < 2 min, and/or who have 5 or more missing responses will be excluded;
```{r}
DF.idaq %>% filter(duration <= 2.3) %>% group_by(check) %>% count()
DF.idaq %>% filter(is.na(response)<= ) %>% group_by(check) %>% count()

is.na(titanic_train)

tapply(DF.idaq %>% filter(duration <= 2.3,length))
...
Participants who have 5 or more missing responses for the IDAQ questionnaire will be excluded
```{r}
DF.idaq <- DF.idaq %>%
  filter(duration <= 2)
