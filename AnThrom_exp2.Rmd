...
```{r}
tom_combined <- bind_rows("effectance" = as_tibble(as.mcmc(model1ef,  pars = c("cent_sitant", "cent_sitant2", "cent_disant", "cent_disant2"), combine_chains = TRUE)),
                          "falsebelief" = as_tibble(as.mcmc(model1fb, pars = c("cent_sitant", "cent_sitant2", "cent_disant", "cent_disant2"),combine_chains = TRUE)),
                          .id = "tom") %>%
  rename(linear_situational = cent_sitant, quadratic_situational = cent_sitant2, linear_dispositional = cent_disant, quadratic_dispositional = cent_disant2) %>%
...
```{r}
model1ef_i<- comp_model("model1ef_i", cent_effectance ~ 1) 
model1ef_s<- comp_model("model1ef_s", cent_effectance ~ cent_sitant) #prediction 1
model1ef_d <- comp_model("model1ef_s", cent_effectance ~ cent_disant2) #prediction 2
model1ef_sd <- comp_model("model1ef_s", cent_effectance ~ cent_sitant + cent_disant2) #prediction 3
```

## 10.2 Incremental models for false belief
We start with the simple intercept-only model, followed by a linear situational predictor model, and a quadratic dispositional predictor model, and a combined model/  
```{r}
model2fb_i<- comp_model("model2fb_i", cent_effectance ~ 1) 
model2fb_s<- comp_model("model2fb_s", cent_effectance ~ cent_sitant) #prediction 1
model2fb_d <- comp_model("model2fb_s", cent_effectance ~ cent_disant2) #prediction 2
model2fb_sd <- comp_model("model2fb_s", cent_effectance ~ cent_sitant + cent_disant2) #prediction 3
```
