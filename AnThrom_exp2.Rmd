...
DF.demo <- list.files("experiment2/data/pilot",pattern="\\prolific*", full.names=TRUE) %>% #change to "experiment2/data/main"
  map_dfr(read_csv)
```

## 2.2 Load the psychopy/pavlovia files
```{r, echo = TRUE, message = FALSE}
DF.exp2 <- list.files("experiment2/data/pilot",pattern="\\PARTICIPANT*", full.names=TRUE) %>% 
  map_dfr(read_csv) %>% 
...
```{r}
DF.rating <- DF.exp2 %>%
  select("participant_id", "eventnrM","event_typeM", "measure", "item", "focus", "key_resp_1.keys", "key_resp_2.keys", "key_resp_1.rt", "key_resp_2.rt")  %>%
...
excluded_participants <- anti_join(idaq_sub, fa_sub, by = "participant_id") %>%  mutate(reason = "incomplete") %>%
  bind_rows(., excluded_participants)
```
...
DF.mean <- DF.rating %>% 
  mutate(event_nr = factor(event_nr, levels=c(1:16), ordered=TRUE)) %>% 
  dplyr::group_by(event_nr, measure) %>%
  dplyr::summarise(rating = mean(response)) 

p1 <- DF.rating %>%
  mutate(event_nr = factor(event_nr, levels=c(1:16), ordered=TRUE)) %>% 
  dplyr::group_by(participant_id, event_nr, measure) %>% #focus and event_type are included or excluded based on Model C1/C2
...
p2 <- DF.rating %>%
  dplyr::group_by(participant_id, event_type, focus, measure) %>% #focus and event_type are included or excluded based on Model C1/C2
  dplyr::summarise(rating = mean(as.numeric(response))) %>% 
...
```{r}
DF.rating <- DF.rating %>% 
  dplyr::group_by(participant_id, measure) %>% #focus and event_type are included or excluded based on Model C1/C2
  dplyr::summarise(rating = mean(as.numeric(response)))  
```
...
## 8.2 Visualise the relationship
...
## 8.4 Full Model 1 - anthropomorphism and effectance

Test the relationship between situational and dispositional anthropomorphism and effectance:
```{r}
model1ef <- brm(formula = cent_effectance ~ cent_sitant + cent_sitant2 + cent_disant + cent_disant2, 
      data = DF.exp2,
      family = gaussian,
      prior = priors1,
      chains = 4,
      iter = 4000,
      seed = 42, 
      file = "experiment2/models/model1ef.RDS")
```
...
## 8.5 Full Model 2 - anthropomorphism and false belief

Test the relationship between situational and dispositional anthropomorphism and false belief:
```{r}
model2fb <- brm(formula = cent_falsebelief ~ cent_sitant + cent_sitant2 + cent_disant + cent_disant2, 
      data = DF.exp2,
      family = gaussian,
      prior = priors1,
      chains = 4,
      iter = 4000,
      seed = 42, 
      file = "experiment2/models/model2fb.RDS")
```
...
# 9. Posterior distribution {.tabset .tabset-fade .tabset-pills}
...
```{r}
tom_combined <- bind_rows("effectance" = as_tibble(as.mcmc(model1ef,  pars = c("b_cent_sitant", "b_cent_sitant2", "b_cent_disant", "b_cent_disant2"), combine_chains = TRUE)),
                          "falsebelief" = as_tibble(as.mcmc(model2fb, pars = c("b_cent_sitant", "b_cent_sitant2", "b_cent_disant", "b_cent_disant2"),combine_chains = TRUE)),
                          .id = "tom") %>%
  dplyr::rename(linear_situational = b_cent_sitant, quadratic_situational = b_cent_sitant2, linear_dispositional = b_cent_disant, quadratic_dispositional = b_cent_disant2) %>%
  pivot_longer(c("linear_situational", "quadratic_situational", "linear_dispositional", "quadratic_dispositional"),names_to = "varAnthro")
```
...
p9 <- tom_combined %>%
  ...
  dplyr::rename(predictor = varAnthro) %>%
  ggplot(., aes(value, fill = predictor)) +
  geom_density(alpha = .5)+
  geom_vline(xintercept = 0, color = "black", linetype = 2) +
  ggtitle(paste("Posterior distributions for the Theory-of-Mind measures")) + 
  facet_wrap(~tom, ncol =1) +
  theme(strip.background = element_blank()) +
  scale_fill_manual(values=c("#0072B2", "#000033", "#D55E00", "#660000"))
p9
```

# 10. Model comparison {.tabset .tabset-fade .tabset-pills}

We will run several models following the predictions:
    
    1.	There will be a linear and positive relationship between situational anthropomorphism and Theory-of-Mind (model1ef_s and model2fb_s). 
    2.	There will be a quadratic relationship between dispositional anthropomorphism and Theory-of-Mind (model1ef_d and model2fb_d).
...
```{r}
comp_model <- function(name, model_formula, priors_def){
...
## 10.1 Incremental models for effectance
We start with the simple intercept-only model, followed by a linear situational predictor model, and a quadratic dispositional predictor model, and a combined model/  
```{r}
model1ef_i <- brm(formula = cent_effectance ~ 1,
      data = DF.exp2,
      family = gaussian,
      chains = 4,
      iter = 4000,
      seed = 42, 
      file = "experiment2/models/model1ef_i.RDS")

model1ef_s <- comp_model("model1ef_s", cent_effectance ~ cent_sitant) #prediction 1
model1ef_d <- comp_model("model1ef_d", cent_effectance ~ cent_disant2) #prediction 2
model1ef_sd <- comp_model("model1ef_sd", cent_effectance ~ cent_sitant + cent_disant2) #prediction 3
```

## 10.2 Incremental models for false belief
We start with the simple intercept-only model, followed by a linear situational predictor model, and a quadratic dispositional predictor model, and a combined model/  
```{r}
model2fb_i <-  brm(formula = cent_falsebelief ~ 1,
      data = DF.exp2,
      family = gaussian,
      chains = 4,
      iter = 4000,
      seed = 42, 
      file = "experiment2/models/model2fb_i.RDS")

model2fb_s<- comp_model("model2fb_s", cent_falsebelief ~ cent_sitant) #prediction 1
model2fb_d <- comp_model("model2fb_d", cent_falsebelief ~ cent_disant2) #prediction 2
model2fb_sd <- comp_model("model2fb_sd", cent_falsebelief ~ cent_sitant + cent_disant2) #prediction 3
```

## 10.3 Effectance comparison

```{r}
modelcomp_ef <- loo(model1ef_i, model1ef_s, model1ef_d, model1ef_sd, model1ef)
save(modelcomp_ef, file="experiment2/models/modelcomp_ef.RData")
load('experiment2/models/modelcomp_ef.RData')
modelcomp_ef
```

## 10.4 False belief comparison

```{r}
modelcomp_fb <- loo(model2fb_i, model2fb_s, model2fb_d, model2fb_sd, model2fb)
save(modelcomp_fb, file="experiment2/models/modelcomp_fb.RData")
load('experiment2/models/modelcomp_fb.RData')
modelcomp_fb 
```

# 11. HDI+ROPE decision rule {.tabset .tabset-fade .tabset-pills}

## 11.1 HDI+ROPE function
...
## 11.2 Full effectance model
```{r}
hdi_rope(model1ef) 
```

## 11.3 Winning effectance model
```{r, eval = FALSE}
hdi_rope(modelef_winning) #change the name based on LOO (remove eval = FALSE)
```

## 11.4 Full false belief model
```{r}
hdi_rope(model2fb) 
```

## 11.5 Winning false belief model
```{r, eval = FALSE}
